...
- ⏳ Resto (**050+ BLOQUEADO por la firma humana del gate 047**): 050/051 tools lectura → 052 multi-org → 053 crear-borrador+deep-link → 054 DCR/discovery → 055 consent pulido → 056 rate-limit/idempotency tools (atar #061/#062) → 057 provider mcp-public → 058 docs → 059 deploy (crear proyecto Dokploy mcp.tufacturaia.com + DNS). (060 Fase 2).
...
> Continúo el MCP server de TuFacturaIA. Worktree `/Users/manueldelmonte/facturaia-mcp` (rama `feat/mcp-server`), NO el repo principal. Lee `issues/mcp-server-prd.md`, `services/mcp-server/SECURITY-AUDIT-047.md`, `[[facturaia-mcp-server]]` y `[[ADR-032-mcp-oauth-as-split-app-servicio-handroll]]`. **Núcleo OAuth COMPLETO + auditado, TDD**: 041, 044, 043 (mig 327), 042 (skeleton), 045 (/authorize+consent app → /token servicio = user-token 041), 046 (refresh rotación/reúso/revocación + /revoke + claim gid), 048 (resource server verifica AS + corta grant revocado), **047 gate de auditoría PASADO** (4 revisores indep., 0 P0, fixes aplicados, mig 328 `oauth_revoked_grants`, E2E `oauth-e2e.test`). ~2729 tests verde. 049 (rol ∩ scope) hecho. **Diferido: issues 061-063.** Siguiente: **050/051** (tools de lectura de facturas/resto sobre la v1 user-aware) — **PERO 050+ está BLOQUEADO hasta la firma humana del gate 047** (`SECURITY-AUDIT-047.md`). Si el gate aún no está firmado, NO empieces 050; en su lugar revisa el gate o haz 058 (docs) que no expone datos. TDD por slices, commit por slice. Antes de tocar nada: `cd /Users/manueldelmonte/facturaia-mcp && git log --oneline -16 && npm test`. NO subir `MCP_PUBLIC_ENABLED` a true sin la firma.
