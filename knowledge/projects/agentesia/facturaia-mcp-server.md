...
- ✅ **045 AS OAuth core COMPLETO** (hand-roll, ADR-032; src/lib/oauth compartido app↔servicio):
  ...
  - **2701/2701 tests, lint+typecheck+build verde.** Pendiente menor: rate-limit en `/authorize` (hoy session-gated; añadir al pulir/047).
- ⏳ Resto: **046** refresh rotación/reúso/revocación → **047 GATE auditoría seguridad (humano, antes de prod)** → **048** wire user-token real en el resource server (whoami con clave pública del AS; revocación↔resource) → 049 rol∩scope → 050/051 tools lectura → 052 multi-org → 053 crear-borrador+deep-link → 054 DCR/discovery → 055 consent → 056 rate-limit → 057 provider mcp-public → 058 docs → 059 deploy. (060 Fase 2).
...
> Continúo el MCP server de TuFacturaIA. Worktree `/Users/manueldelmonte/facturaia-mcp` (rama `feat/mcp-server`). Lee `issues/mcp-server-prd.md`, `services/mcp-server/README.md`, `[[facturaia-mcp-server]]` y `[[ADR-032-mcp-oauth-as-split-app-servicio-handroll]]`. Hechos+TDD: 041, 044, 043 (mig 327), 042 (skeleton), **045 completo (AS OAuth core hand-roll: /authorize+consent en app, /token en servicio emite user-token 041; src/lib/oauth)**. Siguiente: **046** (refresh rotación + detección de reúso → revoca familia + revocación por grant efectiva, sobre `oauth_refresh_tokens`) y luego **048** (resource server verifica el user-token real con la pública del AS + check de revocación; dev-token solo en dev). TDD por slices, commit por slice. Antes: `cd /Users/manueldelmonte/facturaia-mcp && git log --oneline -10 && npm test`.
