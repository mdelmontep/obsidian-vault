...
- Dos bugs previos del equipo, abiertos como issues: reinvitar mientras el invitado acepta lo devuelve a `invitado` (#2960), y un miembro en `pausa` se reinvita sin la confirmación del revocado (#2961). Test del camino `force_reactivate` (propietario que reactiva a quien sí estuvo activo = invitación nueva, no reenvío) mergeado en #2965, mutación verificada.
...
- #2960 y #2961 cerrados en #2983 (086929b6e): la reinvitación escribe condicionada al estado leído (409 `membresia_cambiada` si la persona aceptó entre medias; 23505 del insert concurrente = mismo conflicto) y la mig 945 quita `pausa` del CHECK de `org_members.estado` (0 filas en prod, aplicada y verificada por catálogo antes del merge).
...
- **Remate (3-sep, #2466, mig 819)**: las dos facturas reales `cobrada` por encima del líquido (Borja Galván A2026-0002 +70,29 €, Soraya Sánchez A2026-0002 +37,50 €) eran filas del backfill de la 644 asentadas contra el total fiscal, antes de que la 770 moviera el objetivo al líquido. La migración anula la fila del bruto con motivo e inserta otra `importado` por el líquido; anula antes de insertar para que el trigger no emita un sobre-cobro fantasma; verificación auto-abortante sobre lo que dejó el trigger. Dry-run en ROLLBACK, aplicada a prod antes del merge. Sale con el número 819 porque el 818 de #2454 (abono parcial) ya estaba aplicado en prod sin mergear, y por eso el PR lleva los tipos de `factura_rectificadas`.
...
- Las cinco decisiones de panel, el 3-sep tras un análisis con varios agentes: **gate y anticipos** disueltas con medida en prod (#2449, docs); **heap del typecheck** (#2450); **`ignorado` es una marca, no un estado** (#2459, mig 817: `es_ignorado` en la vista, `p_incluir_ignorados` en las dos RPC, toggle en la vista, `bulk-confirm` los excluye en los dos modos, ADR-076); **la cuenta del CSV la manda el fichero** y se pregunta solo si falta (#2456, mig 816 `conciliacion_cuentas_manuales`, import acepta `cuenta` XOR `cuenta_bidx`). #2460 cerró el registro en `docs/qa/AUD-conciliacion-2026-09-02.md`.
- Lo que costó: víctimas de `mutate` caducadas a las 2 h, el trinquete de diseño al mover botones a `_parts/`, el 816 pisado entre las dos ramas (renumerado a mano al 817, `mig:renumerar` se niega, #2095), un `ADR-069` nacido en el vault a la vez que el del repo (parado por el candado; ahora es el 079) y un `git reset` mixto que dejó 41 ficheros viejos en el árbol.
- Queda: OK a los manuales de los dos últimos, y tres `cobrada` con cobro por encima del importe (las 107,79 € de Borja Galván y Soraya Sánchez del 30-ago, más 135 € de QA1), que parecen pagos brutos sin retención.

## 3-sep-2026 · tickets 169 y 170 de Pescados Chivite (PR #2424 · #2427 · #2428 · #2429 · #2436 · #2437, migs 810/812, ADR-064 + ADR-077)

- **169 (bug)** — las facturas de Wildomar no proponían albarán: el OCR había creado una ficha gemela (`ESB…` junto a `B…`/`nif_iva`), la casación busca por `proveedor_id`, y además sin proveedor `buscarAlbaranesSinCruzar` no avisa. Un solo criterio de resolución (nif → `nif_iva` único, corta en empate → nombre exacto) en los cuatro sitios: `resolve_proveedor` (mig 810, cierre del empate en la 812), `elegirProveedorExistente`, `resolverProveedorPorNombreONif` y `aprobarItem` de `/ingesta`; los dos TS comparten fixtures (`resolutores-equivalentes.test.ts`). Medido en prod: la factura del 21-ago pasa de 4 a 14 pares; en semana, del 41 % al 87 % sobre 11 facturas. Ticket `resuelto`, respondido.
- **170 (pregunta)** — el abono semanal que resta mercancía de una o varias facturas (El Corte Inglés) será una rectificativa por diferencias con origen, nunca un abono suelto ni una anulación: ADR-077 + issue #2426 con la secuencia de PRs; antes del PR 1, `/to-spec`. Respondido, queda `en_revision` a la espera de cómo calculan el abono (artículo, kilos o porcentaje).
- **Gates** — primer `/fia-cierre` sobre los 4 PRs: no-listo (1 bloqueante: el cuarto resolutor, que ni cuatro PRs ni el `/code-review` vieron; 8 avisos). Fix en #2436 con 3 mutaciones con víctima y `npm run gate` entero. Segundo `/fia-cierre`: con-reservas (0 bloqueantes, 4 avisos corregidos en la misma PR), registrado en `docs/plan/cierres.json` (#2437).
- **Sin hacer, con motivo** — el hueco de `buscarAlbaranesSinCruzar` mudo sin proveedor queda fuera del alcance y sin issue; el smoke de aprobación en sandbox no se ejecutó (la bandeja de Obras sandbox no tiene pendientes y no se fabrican filas a mano).
- Artefacto antes/después: https://claude.ai/code/artifact/9954f587-4cef-4378-9449-1c89f8433424 · Aprendizaje ampliado: [[censo-de-llamadas-al-llm-por-el-helper-no-ve-al-sdk-a-pelo]].
...
- ✅ **La auditoría de conciliación queda en 87/87 (4-sep, #2474 · #2490 · #2491 · #2494 · #2495 + el PR documental, migs 829-831 en prod)** — los 35 hallazgos que el plan de septiembre dejó fuera, cerrados en cinco worktrees con juez Opus, constructor Sonnet y revisor Opus: 76 `cerrado` + 11 `no-era`, y el comando de la cabecera del registro imprime 0. Lo que cambió de verdad: la cola del enriquecimiento **excluye en SQL a las orgs que no tienen nada que pedirle al modelo** (mig 829, re-evaluado en cada pasada, no una marca persistida); el presupuesto ya facturado lo impide un **índice único parcial** y el origen se escribe dentro del mismo INSERT (migs 830-831), con un 409 que nombra la factura existente; el cupo PSD2 se **decide y activa en una sola sentencia** (`bank_consent_activar_si_cupo`), y `plan_limit` revoca mientras `no_plan` no, porque el nonce ya se consumió; y el escáner de `db-max-rows` cuenta `>= 1000` en vez de `> 1000` —pedir justo el tope es indistinguible de haber sido recortado—, lo que destapó 22 lecturas más en 17 ficheros sin tocar producción. Abierto solo **#2496**: siete tests de integración que el `describe.skipIf` llevaba meses tapando; los tres mirados a fondo son defectos del test, no del producto (dos escriben columnas que no existen, uno siembra la org sin `verifactu_activo` y luego exige que el trigger la trate como activa). La acción de valor no es arreglarlos de uno en uno, es que la etapa de integración **denuncie cuántos ficheros se salta**.


### Retirado del NOW en el cierre del 5-sep (cerrado, sin trabajo pendiente)
...
### Retirado del NOW en el cierre del 6-sep (narrativa ya cerrada; lo pendiente se quedó arriba, condensado)

- 🟢 **Etapa de integración: medida, exigida por el push y AUDITADA (6-sep)** — #2496 y los seis hallazgos, cerrados en siete PRs ya en prod. **Queda el #2524** (PR 2 de 2: reconstruir de volumen limpio y ver que sin el preludio vuelve el `SIN_DML`; reloj 30-oct). Sueltos **#2561** flaky y **#2562** guard ciego. → [[una-migracion-aplicada-sin-registrar-hace-mentir-a-migration-list]]
- 🟢 **FacturaDirecta: el cursor deja de adelantar a lo que falló (5-sep, #2518)** — se ancla al fallo más antiguo (`seguirCursor`) y el aviso nombra el documento. Al desplegar, las alertas con `dedupe_solo_type` se cierran y reabren una vez. **Tuyo**: el código de impuesto de esas dos líneas, en FacturaDirecta. → [[un-cursor-incremental-que-avanza-sobre-lo-que-fallo-pierde-el-documento]] · [[dedup-key-no-debe-incluir-contenido-volatil]]
- 🟢 **Beta gratuita en prod (3-sep, #2406/#2421/#2425, migs 807/808)** — la fase se cambia en `/admin/leads`; el alta, en `PROMPT-beta.md`. → [[un-smoke-lanzado-antes-de-que-el-autodeploy-termine-mide-el-codigo-viejo]]
- 🟢 **Tickets 166/167/168 en prod y su code review resuelto (2/3-sep, #2408-#2411, mig 805)** — queda #2416 (agente). → [[facturaia-historico-detallado]] §3-sep · [[un-registro-que-estampa-head-vale-solo-con-el-arbol-limpio]]
- 🟢 **Los slides del carrusel ya salen maquetados (2-sep, #2379, migs 793/794/795)** — smoke real verde. **Queda**: lo dispara un botón; el runner todavía no maqueta solo. → [[facturaia-historico-detallado]]
- 🟢 **Ticket 171 de Chivite: guardarraíl en prod e inventario REPARADO (5-sep)** — reparado en una sola transacción y en la fecha original de los apuntes; el «hueco de 42» era un producto contra tres, y bien comparado son −5 y +3 envases. **Ticket CERRADO, no se reabre**; respuesta enviada. **El cliente es JOSÉ, no Borja** (el correo salió mal y Manuel lo corrigió en el hilo). Estado en `PROMPT-continuacion-tk171-cierre-total.md`; ejecución por agentes en `PROMPT-continuacion-tk171-3d2-cabo-y-manuales.md` (6-sep); issue **#2538**. Abierto: 31 lectores + `DROP`, el cabo, manuales, 3 verificaciones, y la fase 4 reabierta a petición de Manuel. → [[el-saldo-historico-se-deriva-hacia-atras-desde-el-actual]] · [[un-guardarrail-mide-lo-que-asienta-el-consumidor-no-lo-que-trae-la-entrada]]
- 🟢 **Abono parcial: 6 de los 7 PRs en prod; el ticket #170 resuelto** (5-sep, #2528, migs 844-846) — la pantalla ya emite la rectificativa por diferencias (reparto por pesos CON IVA, tope bajo el `FOR UPDATE`, lo previsualizado se recomprueba al emitir). Smoke en prod verde. Quedan el **6** (recibidas — leer antes el corolario 4 del ADR-077) y el **7** (API v1), en ese orden de merge. **Tuyo**: decidir si `disputada` vuelve a `pendiente`. Cabo medido y NO tocado: el guard de `stock_rectificar_entrada` no salta tras un abono. → `PROMPT-continuacion-abono-parcial-prs-5-6-7.md` · [[facturaia-historico-detallado]] §5-sep
- 🟢 **La FK de la 845 dejó ambiguo todo embed `facturas ↔ lineas_factura`; arreglado (5-sep, #2529)** — `300 PGRST201` en 10 sitios. El guard de tipos NO tenía la culpa (falsado); queda vivo el **#2530**: un `db push` entre dos pushes de git no pasa por gate ni deja rastro. → [[una-fk-nueva-hacia-una-tabla-ya-referenciada-rompe-los-embeds-de-postgrest]] · [[un-diff-de-fichero-generado-sin-danos-colaterales-no-salio-del-generador]]


### 2026-09-10 — ticket 169, cierre entero (podado del NOW)
...
### 2026-09-10 (noche) — ticket 175, fase A en producción

- 🟢 **Ticket 175 de Chivite: un abono emitido por error ya se anula, EN PROD (10-sep)** — cuatro PRs (#2699 la RPC `abono_anular` + el gesto en el listado, #2705 un cobro ya revertido deja de bloquear, #2706 los tipos generados vuelven a cuadrar con prod, #2708 los cuatro hallazgos del code review) y migs **885/886/887** aplicadas y verificadas por psql. La 887 añadió el bloqueo `registro_externo` (un abono importado se anula en el SIF que lo emitió), la lista de estados en los que `recompute_factura_estado` no hace nada, y un guard que lee el `prosrc` de esa función para que la lista no se pudra → [[un-espejo-entre-dos-funciones-sql-se-ancla-leyendo-el-prosrc-de-la-otra]]. Mensaje de cierre a **José** en el hilo, verificado en la BD (21:44 UTC). El ticket se había marcado `resuelto` a mano a las 19:40, **40 min antes** de que el fix llegara a prod y sin avisar al cliente. Dos cabos vivos en el hub: el bloqueo `registro_externo` sin caso servido para el smoke, y `origenes_no_recomputados`, que se calcula y se registra pero no se enseña.

### 2026-09-12 — estudio de pieza: el modelo por escena y el tope antes de encolar

- 🟢 **#2683 en prod (PR #2731, `5800dd4d7`, 13 ficheros / 1.415 líneas)** — el estudio de `/admin/marketing/contenido` ya deja decidir CON QUÉ se genera cada escena: modelo del catálogo (#2682) y su duración, con el precio de ese modelo y lo que queda del tope del mes delante antes de pulsar. Sin elegir nada, todo cuesta y se genera igual que antes. La lista ofrece **solo lo que el endpoint aceptaría** (de vídeo, aceptado, activo y del proveedor de la pieza); enseñar el resto para que la generación lo niegue con un 409 convertiría el desplegable en una trampa. El endpoint revalida igual: entre pintar la lista y pulsar, un modelo puede dejar de ser servible → [[filtrar-las-opciones-por-un-predicado-convierte-lo-invalido-en-ninguno]]. La fila de auditoría dice ahora **con qué** se generó (`modelo_id`, `duracion_s`, antes y después), no solo cuánto costó: si el gasto lo aprueba quien ve el precio (ADR-087), reconstruir qué aprobó no puede exigir rebuscar en el JSONB del plan.
- **Tres defectos que no estaban en el ticket y salieron escribiéndole los tests** — (1) **la pantalla numera desde 1 y el plan guarda desde 0**, y el panel usaba uno por el otro: la «Escena 1» (el hook, la única que dobla duración y coste) **no se podía editar nunca**, la última decía que no estaba en el plan y las de en medio editaban la de al lado. No lo veía ninguna suite porque el panel «funcionaba»: cargaba, pintaba y encolaba. (2) El panel importaba **el módulo CSS equivocado**, así que dos clases resolvían a `undefined`. (3) **El tope del mes se leía DESPUÉS de escribir**: un take que no cabía se encolaba igual, el runner se negaba a servirlo y para entonces el plan nuevo ya había sustituido al vigente — se perdía el plan bueno a cambio de nada. Ahora el gate corre antes de tocar nada, con el predicado importado de `contenido-cupo.ts`. Alcance declarado: es el gate en ese instante, **no una reserva** (el Gate 2 solo cuenta runs ya reclamados); a lo que se cuele lo para el claim, que es quien manda.
- **Criterio 5 sin cerrar, y no por falta de trabajo**: «un modelo sin precio publicado aprende el suyo de la primera generación» **no es implementable** porque ningún proveedor devuelve el coste de la llamada. Medición y las dos salidas, en el comentario del issue **#2683**; la decisión está en el hub.
- **Prompt de continuación (#2732, `df58562b1`)** — `docs/architecture/PROMPT-continuacion-12-sep-marketing-estudio-de-pieza.md`: la cita de Manuel («debo tener el control»), las 11 issues con su estado medido, el **orden serial #2684 → #2685 → #2722** (comparten `generar/route.ts` y `editar-escena-panel.tsx`) frente a #2686/#2687 en paralelo, y siete trampas (1-based vs 0-based, el runner se despliega primero, el alcance declarado del gate, el veredicto único, la cancelación del hook doble, los espejos TS/SQL, ADR-087). La fila de routing del `CLAUDE.md` apunta ahí.
- **Barrido de ramas**: 23 `feat/marketing-*` medidas; **15 estaban mergeadas** y `--is-ancestor` las daba todas por pendientes → [[git-merge-base-is-ancestor-falso-negativo-con-squash]]. Quedan **7 huérfanas** de verdad (sistema de takes) por rescatar o borrar; sus SHA, en el prompt de continuación. Gate completo verde dos veces: **1.933 ficheros / 20.042 tests**, integración 91/91, e2e:layout 74, build OK.
