...
### Fase B — COMPLETA 2026-06-28 ✅
- **PR-A `#556` ✅** · **PR-B `#559` ✅** · **D3 pleno `#561` ✅** · **conformidad XSD `#570` ✅**
- `#570`: DescripcionOperacion, orden DetalleDesglose, IdSistemaInformatico, namespace → SuministroInformacion.xsd. F1/F2+Destinatarios implementado (FAQ AEAT oficial). 67 tests verdes.
- **Único pendiente**: smoke PRE con .p12 Gonzalo → confirmar namespace envelope SOAP (SuministroLR.xsd split o NS único). Sin orgs con verifactu_activo=true ni certs cargados — sin incidente.
- **Deuda ajena a #3**: cron `slack-dispatcher` en registry sin case en `loadHandler` → test rojo. Equipo Slack.
