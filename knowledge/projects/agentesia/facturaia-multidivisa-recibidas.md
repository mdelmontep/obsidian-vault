...
- **S4 cerrado el 22-ago (#2101): no queda nada del área.** La campanita la emite el propio barrido diario, no un cron aparte (así no avisa de filas que ese mismo barrido está a punto de arreglar): `divisas_pendientes`, agregada por org, `warning`, `expires_at` +36 h, y el conteo con `fetchAllPages` porque ese número lo lee el cliente. **Badge del sidebar**: ya lo servía `counts.inbox` (estas filas son filas de bandeja); un segundo contador sobre el mismo destino competiría con él. **`revisarBandejaMoneda` se retira**: adivinar la divisa por historial iría contra lo que dice el documento, y esa vigilancia ya vive donde está la evidencia (`moneda_baja_confianza`, `moneda_divergente` en `high`). **La notif del desvío >5 % también se retira**: un desvío solo existe cuando alguien teclea un tipo pactado y ya se le avisa bajo el campo en ese instante.
- El «cutover SSOT de WhatsApp» que figuraba pendiente estaba **hecho desde el 10-jul-2026** (receptor de Meta = `src/app/api/whatsapp/webhook/route.ts`, n8n `active:false`). El **ISP** no es deuda de este sprint: es el Sprint P2 de esta misma spec.
- Auditoría de composición de los cuatro PRs: `lib/multidivisa/aprobar-filter.ts` era un guard que solo llamaba su propio test. Retirado en #2101 → [[defensa-cableada-vs-codigo-muerto]]
