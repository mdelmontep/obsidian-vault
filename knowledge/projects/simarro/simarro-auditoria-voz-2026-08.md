...
- v22: tercera vía explícita para "no reconozco nada identificable" — preguntar en vez de decir la palabra suelta. Patrón: [[retell-nodo-conversacional-debe-cubrir-explicito-el-caso-no-entendi]].
- v23 (12-ago tarde): auditada llamada real (`call_140936c164a6284a21def1a09ca`) → 3 fixes: `n_proponer_hora` no puede decir "un agente te contactará" antes de ejecutar `Reservar` (lo decía, adelantando el nodo de cierre); `n_consentimiento` ya no repite la frase idéntica ante audio ininteligible; regla anti-eco en `global_prompt`. Validado con llamada de prueba (`call_bdf8f0951546391a72b73669f93`): fix #1 confirmado, #2 y #3 sin disparar esa rama. El "eco" que motivó el fix #3 resultó ser la prueba hecha en altavoz, no la línea Netelip — [[probar-agente-voz-en-altavoz-genera-eco-que-parece-bug-de-flow]]. De paso, confirmado que `phone` sigue llegando corrupto desde el LLM (`"+34{{from_number}}"`) pero ya blindado en n8n desde antes — [[retell-from_number-no-auto-sustituye-en-tool-args]].
