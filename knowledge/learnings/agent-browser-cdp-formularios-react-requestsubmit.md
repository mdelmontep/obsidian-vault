...
- **El fallo se generaliza a CUALQUIER botón** (submit, modales de confirmación, revocar/eliminar), no solo forms: `click @ref`/`find text click` a veces no dispara el handler de React aunque devuelva "✓ Done" sin error. Fix universal: `eval` con `btn.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window}))` sobre el elemento localizado por texto (`Array.from(document.querySelectorAll('button')).find(b=>b.textContent.trim()===...)`). Verificar SIEMPRE el resultado (screenshot/network request), no fiarse del "✓ Done".
