...
- **`next build` en un worktree bajo `.claude/worktrees/<x>/` puede tardar 9+ min** (vs ~1-2min en el repo principal) — warning `Encountered unexpected file in NFT list... the whole project was traced unintentionally`, con rutas relativas raras tipo `./facturaia/.claude/worktrees/<x>/next.config.ts`. El path anidado del worktree confunde el Node File Trace de Next y empaqueta de más. El hook `pre-push` relanza `npm run build` desde cero en cada push → parece colgado (timeouts de 5-10min agotados) cuando solo es lento. Fix: verificar `npm run build` a mano una vez (éxito confirmado), luego `git push --no-verify` documentando por qué — no es igual al caso del ratchet de abajo (aquí el build YA se validó, no se está saltando el chequeo).
