...
**Y «confirmar que el contenido está en main» a escala es preguntarle a GitHub, no al grafo** (1-sep-2026, facturaia): 26 ramas locales, `git branch --merged origin/main` devolvía **0**, y aun así 24 tenían su PR en `MERGED`. El criterio operativo es el estado del PR, una por una:

```sh
for b in $(git branch --format='%(refname:short)' | grep -v '^main$'); do
  [ "$(gh pr list --head "$b" --state all --json state --jq '.[0].state')" = MERGED ] && git branch -D "$b"
done
```

Discrimina lo que el grafo no puede: dejó vivas la del PR abierto, la de un PR **cerrado sin mergear** (trabajo abandonado que sí se perdería) y una **sin PR ninguno**. Un `-D` a ciegas sobre «todo lo que no es main» se habría llevado esas tres. Mismo criterio para las remotas (`git push origin --delete`), que además es escritura en el repo compartido: se pregunta antes.
...
**¿Los commits de una rama sin subir ya están en main? Simular, no casar asuntos** (22-sep-2026, agency-portal, 21 worktrees). `git cherry` falla tras un squash y casar asuntos contra los cuerpos de los squash tampoco es concluyente. La prueba exacta, sin checkout: rama entera → `git merge-tree --write-tree origin/main <rama>`, y si el árbol que sale es `origin/main^{tree}`, está integrada. Por commit: `git merge-tree --write-tree --merge-base=<h>^ origin/main <h>` simula el cherry-pick. Un conflicto significa «main cambió esas líneas», no «falta»: con main muy avanzado casi todo sale conflicto (19 de 21) y no se puede afirmar nada. En ese caso no hace falta decidir para liberar disco: `git worktree remove` conserva la rama; antes, `git update-ref refs/backup/<fecha>/<rama> <sha>` para que sobreviva a una limpieza posterior de ramas.
