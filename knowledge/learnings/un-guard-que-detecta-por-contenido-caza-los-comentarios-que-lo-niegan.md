...
**1-sep, la variante propia: el candado se lo hace a SÍ MISMO.** Un test de espejo SQL exigía que el predicado ancho (`COALESCE(al.catalogo_id, m.catalogo_servicios_id)`) **no apareciera** en la migración, y lo comprobaba sobre todo el prefijo del fichero. Rojo: la cabecera de la migración cita ese patrón literal para explicar **por qué se rechazó** —con la medición de los 9.567 documentos que habría bloqueado—, o sea que documentar la decisión rompía el candado que la defiende. Fix: medir **el bloque del guard** (de `IF NOT COALESCE(p_permitir_sin_producto` hasta su `ERRCODE`), no el fichero. Una aserción de ausencia necesita un ámbito acotado a código; sobre prosa siempre pierde.

Ver [[recurso-de-test-con-nombre-constante-no-aisla-entre-procesos]] · [[arnes-con-asserts-de-eco-y-falso-verde-no-detecta-nada]] · [[un-candado-que-vive-en-tsc-es-invisible-para-la-suite-y-para-la-mutacion]] · [[el-predicado-de-un-guard-se-mide-contra-el-historico-antes-de-escribirlo]]
