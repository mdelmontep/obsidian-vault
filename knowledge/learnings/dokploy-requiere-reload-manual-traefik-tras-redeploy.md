...
**Dominio NUEVO en compose EXISTENTE: `domain.create`+`reloadTraefik` NO basta — el router
lo materializa el `compose.deploy`** (2026-08-20, apex tufacturaia.com, FB-10): tras crear
los dominios y recargar, el host seguía en **404** (no Bad Gateway: el router no existía).
Orden que funcionó: `domain.create` ×N → `compose.deploy` → esperar el deploy →
`reloadTraefik` (para el cert) → smoke. El reload corta conexiones unos segundos: un smoke
lanzado justo después da `000`, repetir. Certs de apex y www emitidos en <1 min tras eso.
