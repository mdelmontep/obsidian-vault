...
**Casos detectados FacturaIA (mig 120-121)**: `get_user_org_id()` SQL, `getOrgId` y `getSessionActor` TS, `whatsapp-resolve.resolveSenderOrgForUser`, `resolvePhoneChangeMatch` (bucket pending_change_old). Cada uno resolvía org arbitraria — RLS leía/escribía en la "no elegida".
