...
**Gotcha 2026-05-22**: al extender la matriz, PARTIR de la última mig que `CREATE OR REPLACE FUNCTION user_can_write_in_org`, no de la primera. Caso real TuFacturaIA: spec asumía mig 121 pero la vigente era 127 (incluía `catalogo, categoria, module_config, team, series`). Si copias 121 pierdes esos recursos y rompes RLS de `catalogo_servicios` silenciosamente. Antes de extender: `ls supabase/migrations/ | grep -l "CREATE OR REPLACE FUNCTION public.user_can_write_in_org" | sort -V | tail -1` (o `grep -l` dentro de los .sql). El espejo TS debe reflejar la última mig SQL, no la primera.
