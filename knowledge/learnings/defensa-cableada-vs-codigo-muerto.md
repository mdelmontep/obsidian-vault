...
**Ampliación 2026-08-22 (facturaia, auditoría de composición de #2089-#2092): su propio test cuenta como consumidor.** `src/lib/multidivisa/aprobar-filter.ts` era un guard de aprobación («una bandeja `manual_requerido` no se puede aprobar») que **no llamaba nadie**: el grep daba 2 hits —la definición y su test— y eso se lee como «cableado y además cubierto». Lo contrario: el test lo mantenía verde y por eso nunca salió en ninguna suite ni en ningún trinquete. Al borrarlo, `src/lib/multidivisa/` se quedó vacío: el módulo entero era el guard muerto. Regla operativa: al grepear consumidores, **excluir `__tests__` y contar los que quedan** (`grep -rn X src/ | grep -v __tests__`); 0 = muerto, aunque su carpeta tenga tests verdes. Y antes de recrearlo, preguntar dónde vive el filtro de verdad: aquí lo hacía el servidor (422) y el cliente solo lo leía, que es la forma correcta porque el veredicto cambiaba solo con el barrido diario.
