...
- **`pg_dump` del host se niega a volcar de un server de versión MAYOR** (`server version mismatch`) → corre `pg_dump`/`psql` DENTRO del contenedor (`docker compose exec -T postgres`) para paridad de versión. Si el entorno NO trae el plugin `docker compose` (v2) ni `docker-compose` (v1) — pasa en algunos entornos Claude Code —, usa `docker exec <contenedor-en-marcha>` directo (reutiliza el pg ya levantado) y replica a mano la lógica de `check-drift.ts` (crea drift_a/drift_b, aplica schema.sql vs baseline+migraciones, dump+`compareSchemas`).
