...
Reglas:
- **NO añadir `bash -c` manualmente al Command** (confirmado 2026-05-11). Dokploy ya lo envuelve desde el dropdown Shell. Doble wrap → `bash -c curl -X POST ...` interpreta `curl` como script y `-X` como `$1` → curl falla con args inválidos. Caso real TuFacturaIA: cron `cashflow-alerts` fallaba con "Command failed" hasta quitar `bash -c` del campo Command.
