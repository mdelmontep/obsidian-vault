...
**Patrón de salida limpio**: si `main` divergió Y la sesión paralela está mutando la rama compartida (la contamina con commits ajenos), no pelees el merge in-place — aísla: `git worktree add <tmp> -b <rama> origin/main` + `cherry-pick <commit>` + `push --no-verify` (worktree sin `node_modules` → el gate build no corre, pero el contenido ya estaba validado en local) + PR. Lleva solo tu commit a main sin arrastrar WIP ajeno.
