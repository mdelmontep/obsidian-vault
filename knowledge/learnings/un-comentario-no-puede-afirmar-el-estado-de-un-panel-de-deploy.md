...
📄 **Y lo mismo vale para un DOCUMENTO entero, no solo para un comentario** (18-sep, agh-iberica). Una página que explicaba el piloto a la parte cliente decía «faltan worker, validadores y propuesta conectada a la interfaz»; esa misma noche entraron la vista de la propuesta y el despliegue de los tres workers. Un documento que **afirma estado** no se arregla con voluntad de mantenerlo: o alguien lo actualiza en cada merge (trabajo recurrente que nadie asigna) o se lee meses después como si fuera verdad. La salida barata es **cambiarle el tiempo verbal por una fecha**: va a la carpeta *write-once* (aquí `docs/status-log/`), con la fecha en el **nombre**, en el `<title>` y en un aviso dentro de la página que remita al snapshot vivo. Así «esto era cierto el 15-sep» es correcto para siempre y su coste de mantenimiento es cero.
