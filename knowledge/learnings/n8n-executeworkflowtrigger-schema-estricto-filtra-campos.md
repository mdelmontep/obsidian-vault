...
Caso real Elphis 2026-09-14: el fix del dedup de avisos por llamada (`notif_scope = dec.call_id || dec.phone`) llevaba 5 días escrito y **muerto**: `call_id` no estaba en `Recibir inputs` ni lo mandaba ningún llamante, así que el `||` caía siempre al teléfono y un paciente que llamó dos veces se quedó sin segundo aviso. Un fallback en el código tapa el campo filtrado: el código «funciona» con el valor de reserva. Tell: abrir `Recibir inputs` de una ejecución real, no leer el nodo que lo consume.
