...
- **El orquestador es el checker REAL, no te fíes del auto-verify del agente.** Su "tsc clean" fue incompleto (se coló un `.select` sin columnas que el mapper usaba → tsc rojo). Tras CADA lote: `tsc --noEmit` COMPLETO + tests + grep del diff por red flags (`as any`/`@ts-ignore`/`: any`/`as unknown as`). Los agentes verifican con `eslint scoped` + tsc puntual (project-wide tarda por contención).
