...
**Afinado (2026-09-03, mig 819 / #2466): la copia del fichero ajeno va SIN commit.** Traerlo con commit mete en tu PR una migración que su PR aún no ha mergeado. Basta con que exista en disco mientras corre el CLI: `git show origin/<rama>:supabase/migrations/NNN_*.sql > supabase/migrations/NNN_*.sql`, `db push --dry-run` (solo la tuya), `db push --yes`, `rm`. El CLI compara versiones, no git. Lo que SÍ te obliga a llevar es `database.types.ts` regenerado: `gen:types:check` del pre-push compara con prod, y prod ya tiene la tabla de la migración ajena; se lleva y se dice en el cuerpo del PR (+73 líneas de `factura_rectificadas` de #2454).
