...
**Medido a escala (14-ago, tanda de 15 PRs)**: de 8 subagentes a los que se les encargó el gate completo (con la instrucción de foreground y el aviso del semáforo), **murieron los 8** por `stalled: no progress for 600s` — unos con el build a medias, otros con el commit hecho y sin pushear. Los que solo editaban terminaron todos. El reparto que funciona no es "instrúyelos mejor", es **a los agentes la edición y al orquestador los gates** en `run_in_background`, que no tiene watchdog de stream. Coste añadido: sus cambios quedan sin commitear en el worktree, y hay que ir a mirar `git status` de cada uno antes de dar nada por perdido.
