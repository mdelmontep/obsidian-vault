...
- [[dos-castigos-por-el-mismo-evento-hacen-inalcanzable-el-estado-bueno]] — dos guardas que castigan el mismo evento hacen inalcanzable el estado que protegen
...
- [[permiso-granular-sin-ui-de-concesion-se-queda-en-una-fila]] — un permiso fino sin pantalla para concederlo acaba con una sola fila y el resto viendo 403…
...
- [[cuenta-los-motores-que-calculan-el-mismo-numero-antes-de-arreglar-uno]] — cuenta los motores que calculan el mismo número antes de arreglar uno
...
- 2026-08-18 · TuFacturaIA · **cinco PRs mergeados, y los dos que no estaban planeados salieron de arreglar lo que rompieron los otros tres**: `db push` de la mig 708 murió a mitad contra prod con un 23514 porque copió el formato de serie del `DEFAULT` del `001_schema.sql`, invalidado por la mig 021 meses antes (el fichero histórico sigue mintiendo; el default vivo en prod ya era otro). Sin daño: `BEGIN`/`COMMIT` explícitos → rollback total y prod intacta en la 707, verificado por catálogo. Y la suite completa sobre `main` mergeado volvió a pagar **por segundo día seguido**, esta vez con un guard **estructural** (toda FK bloqueante a `facturas` la debe desenganchar `recibida_eliminar`; la tabla nueva de la 708 no estaba) — el `pre-push` no ejecuta los tests, así que ningún gate por rama lo veía. Ver [[default-del-schema-inicial-puede-estar-invalidado-por-un-check-posterior]] · [[suite-filtrada-por-carpetas-del-pr-no-ve-los-guards-de-arquitectura]]
- 2026-08-18 (tarde) · TuFacturaIA · **once PRs en el día, y los dos incidentes de la tanda los causé yo con herramientas del propio repo**: `gen:types` truncó el fichero de tipos antes de fallar (401 de la Management API) y un `\i` de migración dentro de un `BEGIN … ROLLBACK` de prueba ejecutó su `COMMIT` interno y dejó una función creada en producción. Los dos medidos y cerrados: el script ahora genera a temporal + valida + mueve, con test que prohíbe la redirección y `--check` de drift; la función quedó registrada aplicando su migración por `db push`. De la tanda salen además un trinquete nuevo (`series-formato-guard`, que bloquea un formato de serie que el CHECK rechaza) y dos decisiones escritas: el PR 2 de #1778 **no se escribe** contra un fixture inventado porque no hay payload real de Stripe ni SDK que valide la forma (§13 del plan), y la serie protegida va en el DATO porque `SERIES_RESERVADAS` se deriva de `SERIE_BY_TIPO`. Y el aviso que más vale del día: la nota del `>` estaba escrita DOS veces desde el 7 y el 11 de agosto y no impidió nada, porque nadie había cambiado el script. Ver [[psql-i-de-una-migracion-ejecuta-su-commit-y-rompe-tu-rollback]] · [[una-lista-derivada-no-admite-excepciones-la-marca-va-en-el-dato]] · [[default-del-schema-inicial-puede-estar-invalidado-por-un-check-posterior]]
- 2026-08-19 · TuFacturaIA · **el issue describía 1 de 2 mitades y 1 de 3 patas, y el gate cazó cuatro claims míos sin respaldo**: #1856 decía que al embudo de reclamación le faltaba sumar el ledger, y lo que faltaba eran dos patas de tres (ledger `factura_pagos` y resto de conciliación) **en dos motores distintos** — la función SQL del cron y su espejo TS, que no se typechequean entre sí. Arreglar solo el lado que nombraba el issue habría cerrado el PR con el cron reclamando de más. Las dos mitades delegan ya en `factura_cobros_resumen` (mig 711, aplicada y verificada por catálogo; daño cero medido en **las dos** exposiciones, no en una). Y el `/fia-cierre` encontró que cuatro afirmaciones del propio PR iban por delante de su evidencia, entre ellas un comentario mío que vendía 8 vectores de **aritmética constante** como si discriminasen la versión vieja. Ver [[cuenta-los-motores-que-calculan-el-mismo-numero-antes-de-arreglar-uno]] · [[un-control-negativo-que-no-discrimina-invalida-el-test-entero]]
- 2026-08-24 · Elphis Psicología · **adaptador de agenda GCal desplegado con idempotencia doble** (Redis INCR/TTL + bookingKey en extendedProperties releído antes de crear) y tres gotchas medidos por el camino: el nodo GCal no sabe de extendedProperties (HTTP Request con credencial predefinida), el nodo Redis no tiene SET NX (INCR con expire como lock) y un `echo | jq` de zsh convirtió dos respuestas 200 en "fallos" que duplicaron workflows. Ver [[nodo-gcal-de-n8n-no-soporta-extendedproperties]] · [[lock-e-idempotencia-en-n8n-con-redis-incr-sin-set-nx]] · [[parse-roto-de-una-respuesta-200-se-confunde-con-fallo-y-duplica]]
- 2026-08-25 · agency-portal / Flota IA · **cerrada la fase 1 y con ella dos verificaciones que llevaban meses sin ejercerse**: la firma del webhook de Retell rechazaba el 100 % de las entregas desde el 22-jun (se reescribió a mano al desaparecer `webhook_auth` del SDK 5.38, firmando solo el cuerpo en vez de `cuerpo + timestamp`) y no se notó porque el cron lo tapaba y desde mayo ningún agente emite; y el CI de la org lleva roto desde el 17-ago muriendo en 3 s sin ejecutar un paso, también en `main`. Los cuatro canales tienen ya adaptador con fixture real capturado, los emisores n8n de `kommo_n8n`/`custom_api` viven versionados en el repo y se ejecutan en CI, y el tren de PRs apiladas se partió al mergear las de en medio (`add/add` sobre una migración ya en `main` → se resuelve a favor de `main`). Ver [[una-verificacion-que-nadie-ejerce-puede-llevar-meses-rota]] · [[un-check-que-muere-en-segundos-sin-ejecutar-pasos-es-la-plataforma]] · [[el-emisor-que-vive-fuera-del-repo-se-ancla-versionandolo-y-ejecutandolo-en-ci]] · [[una-fila-de-auditoria-por-corrida-en-multi-tenant-es-invisible-para-el-resto]]
- 2026-08-26 · TuFacturaIA · **el albarán deja de ser cosa de Obras y gana su puente a la factura** (#2209, mig 754, ADR-029): tres tickets de Pescados Chivite que eran el mismo agujero. El albarán mueve stock al validarse y la factura que lo agrupa no lo repite, con el guard **dentro de las dos funciones de Postgres que asientan compras** en vez de en el endpoint, porque ese `if` se esquiva desde el copiloto, la API v1, el OCR o un cron; el eje de facturación no es columna, es vista; y el caso de los dos documentos ya asentados se rechaza (`AL014`) en lugar de duplicar inventario en silencio. Tres cosas mordieron por el camino: siete guards de arquitectura rojos de golpe solo al hacer `git add -A`; un panel injertado en el modal de otra feature que tumbó ocho tests suyos; y el smoke sobre producción que daba el guard por probado sin ejercerlo (la línea de la factura no traía `catalogo_id`, así que la función ni llegaba a preguntar por el albarán). En el mismo recorrido salió un fallo de pantalla: el detalle enseñaba un guion donde iba el producto del catálogo (#2222). Ver [[un-smoke-sale-verde-sin-ejercer-el-guard-si-el-dato-no-cumple-su-precondicion]] · [[un-panel-nuevo-dentro-de-un-modal-ajeno-hereda-sus-tests]] · [[reserializar-un-json-tracked-lo-reescribe-entero]] · [[gate-por-git-ls-files-no-ve-un-fichero-nuevo-sin-git-add]]
- 2026-08-31 · AGH Ibérica / Paquita · **el email dictado por voz era invisible para la red de seguridad que existía justo para eso** (#1502): el backstop de #115 buscaba una `@` literal y el ASR escribe «arroba» — mismo mecanismo que #246 con el teléfono, un campo más tarde. Dos cosas más salieron del camino: el barrido de mutación daba **3 SIN VÍCTIMA** que no eran huecos de test sino **ramas inalcanzables** (dos regex complementarias por construcción), y se cerraron **borrando el código, sin añadir un solo test**; y al medir las flags del contenedor de prod apareció el **cuarto sitio** del patrón de #1331 —un fichero del repo afirmando un estado de prod falso—, esta vez en el `CLAUDE.md`, que se carga en cada llamada de cada sesión y presentaba la frase caducada como *hecho medido que dirige el trabajo*. Ver [[un-detector-escrito-para-la-forma-escrita-es-ciego-a-la-dictada]] · [[un-sin-victima-del-barrido-puede-ser-una-rama-muerta]] · [[un-comentario-no-puede-afirmar-el-estado-de-un-panel-de-deploy]]
- 2026-09-03 · TuFacturaIA · **el preview del reel dejó de ser una imitación del render** (#2423): el overlay existía dos veces —CSS a mano en el panel y filtros `drawtext` de ffmpeg— y divergían sin que nada avisara, así que se aprobaba una composición y se publicaba otra. Ahora se rasteriza a PNG y las dos superficies montan el mismo HTML. Por el camino, tres cosas medidas y no supuestas: el `sandbox=""` del iframe dejaba el preview en **origen opaco**, tumbando sus webfonts en silencio y haciéndole medir con `sans-serif` la altura del cierre; la firma de IA con los 60 caracteres que el schema admite **se salía del lienzo** (518 píxeles de tinta en el margen, con el test escrito ANTES del fix); y `e2e:layout` estaba **fuera de `npm run gate`**, así que llevaba en rojo midiendo un CSS ya borrado sin que ningún verde lo delatara. Ver [[iframe-sandbox-vacio-deja-el-documento-en-origen-opaco]] · [[ci-oom-typecheck-build-heap-explicito]]
- 2026-09-03 · Centro Elphis · **Doctoralia ya actualiza el lead, y una prueba avisó a recepción**: el POST de deals de Clientify ignora `pipeline` y un PATCH «parcial» expira el deal; una rama leía `$input` detrás de un Postgres que emite `{success:true}` (inerte desde el 15-ago); un flag fuera del schema del trigger convirtió la primera prueba en un aviso real; y dos agentes «failed» por 529 habían terminado el trabajo. Ver [[clientify-post-deals-ignora-pipeline-y-un-patch-parcial-reevalua-etapa-y-vencimiento]] · [[un-agente-que-reporta-fallo-por-529-puede-haber-terminado-el-trabajo]] · [[n8n-executeworkflowtrigger-schema-estricto-filtra-campos]] · [[el-nodo-postgres-emite-success-true-cuando-el-returning-sale-vacio]] · [[ADR-079-una-reserva-de-doctoralia-mueve-todos-los-deals-abiertos-del-lead]]
- 2026-09-07 · Simarro · **cuatro fallos que llegaban al cliente, cerrados en producción**: las visitas de dos semanas al año se guardaban una hora antes porque el offset de Madrid se deducía del número de mes (en 3 nodos, encontrado por grep y no por relectura); el recheck previo a reservar enumeraba motivos de fallo en vez de exigir el éxito, así que se agendaban sábados y fechas pasadas; el agente de voz prometía la cita inexistente porque el `global_prompt` mandaba lo contrario que la rama nueva del flow; y "garaje" solo existía como característica de una vivienda, nunca como el inmueble buscado. Se descartó explícitamente mover la creación del lead detrás de la validación: borraría la ficha de quien llama y no logra cita, que es una oportunidad comercial, no basura. Ver [[offset-de-zona-horaria-por-numero-de-mes-desfasa-una-hora-las-citas]] · [[un-guard-que-enumera-los-motivos-de-fallo-deja-pasar-los-que-no-enumera]] · [[una-rama-nueva-del-flow-no-manda-si-el-prompt-global-dice-lo-contrario]] · [[una-palabra-puede-ser-caracteristica-o-el-inmueble-en-si]]
- 2026-09-14 · AGH Ibérica · **PR-2 del piloto de RRHH en `main` tras nueve revisiones (#1738)**: una PR de media jornada creció a 70 commits porque un lector propio de PDF decidía la visibilidad y el bucle de revisión no tenía criterio de parada; un `instanceof` de la clase padre rechazaba DOCX válidos; y Actions (2 vCPU) destapó tests de cupo que dependían del reloj. Ver [[una-revision-adversaria-en-bucle-necesita-criterio-de-parada-escrito-antes]] · [[instanceof-de-la-clase-padre-se-traga-la-subclase-con-otra-semantica]] · [[un-test-que-exige-ok-dentro-de-un-plazo-mide-la-cpu-del-runner]]
- 2026-09-15 · AGH Ibérica · **PR-A del extractor de contratos en `main` (#1755) y el contrato real resultó vectorizado**: la capa de texto solo traía los rótulos, así que decidir por «¿tiene texto?» no discrimina; y un tablero republicado seguía sirviendo datos personales desde su historial de versiones. Ver [[pdf-escaneado-sin-capa-de-texto-renderizar-paginas-con-pdftoppm]] · [[republicar-un-artefacto-no-purga-sus-versiones-anteriores]]
