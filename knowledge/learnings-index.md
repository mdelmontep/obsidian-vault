...
- 2026-08-26 · TuFacturaIA · **el albarán deja de ser cosa de Obras y gana su puente a la factura** (#2209, mig 754, ADR-029): tres tickets de Pescados Chivite que eran el mismo agujero. El albarán mueve stock al validarse y la factura que lo agrupa no lo repite, con el guard **dentro de las dos funciones de Postgres que asientan compras** en vez de en el endpoint, porque ese `if` se esquiva desde el copiloto, la API v1, el OCR o un cron; el eje de facturación no es columna, es vista; y el caso de los dos documentos ya asentados se rechaza (`AL014`) en lugar de duplicar inventario en silencio. Dos cosas mordieron por el camino: siete guards de arquitectura se pusieron rojos de golpe solo al hacer `git add -A`, y un panel injertado en el modal de otra feature tumbó ocho tests suyos por asumir la forma de su respuesta. Ver [[un-panel-nuevo-dentro-de-un-modal-ajeno-hereda-sus-tests]] · [[reserializar-un-json-tracked-lo-reescribe-entero]] · [[gate-por-git-ls-files-no-ve-un-fichero-nuevo-sin-git-add]]
