...
- **Producción**: facturaia.agentesia.world (Dokploy) → migración a `tufacturaia.com` planificada · spec [[facturaia-migracion-tufacturaia-com]]
...
- **🔴 P0 inmediato — rotar OpenAI key expuesta en repo** — `n8n/workflows/whatsapp-receptor-v2.json:~1237` contiene `sk-proj-G__taTj8N_kkUIdFO6...` literal. Revocar OpenAI dashboard → nueva key → PUT workflow `zYcHHa8jWXB6dY5i` con `{{$env.OPENAI_API_KEY}}` → grep marker post-PUT → borrar archivo + commit. Histórico git: filter-repo o asumir compromiso (key ya revocada). Bloqueante para migración tufacturaia.com
- **Migración a `tufacturaia.com` + Dokploy nuevo** — plan 9 fases en [[facturaia-migracion-tufacturaia-com]]. Aprovecha para resolver 5 bugs estructurales: Traefik reload manual, rate-limit in-memory, crons sin lock distribuido, n8n SPOF, Puppeteer sin pool. Decisiones pendientes con Manuel: host nuevo VPS o mismo, `api.` subdominio o monolítico, reusar BD n8n via pg_dump, Resend custom domain ahora o hardening F8
