...
## Estado (2026-08-07, madrugada) — `main` en `22763b9`; **diez merges en una noche** y lo que bloquea sigue siendo HUMANO

✅ **Tren de diez, de uno en uno y con el gate re-corrido entre cada merge** (override de founder documentado en cada PR): #1006/**#981** · #1004/**#1003** · #1012/**#1010** · #1011/#1002-mitad · #995/#895-mitad · #1008 · #1013/**#1005** · #1014/#1007-3-de-4 · #1016. Un merge **propio** invalida la línea de la siguiente igual que uno ajeno — por eso se re-corre, no por burocracia.

🎙️ **El hallazgo del día salió de una LLAMADA, no de leer código.** Manu oyó «Vale, **crear la tarea** «Llamar a Dragados»» y al medir el tono apareció el bug de verdad: el «hecho» de **voz** decía el infinitivo mientras el mismo write por WhatsApp decía «cliente «X» **creado**». La copia en pasado existía desde #575 y **el camino de voz nunca la recibía** — se construía sus líneas aparte para saltarse una agrupación, y con ella se dejó el pasado. Arreglado con **una sola** fuente de la cascada (#1003).

🔴 **Y otro vivo en prod desde que se encendió el presenter:** podía convertir «3 tareas **vencidas**» en «3 tareas **para hoy**» y el guard lo aprobaba — otra pregunta, misma respuesta aparente (#1010). De ahí sale la decisión de fondo: → [[ADR-050-el-modelo-frasea-solo-donde-el-guard-puede-rechazarlo]]. Precio medido antes de aceptarla: de 11 lecturas, **8 siguen fraseándose**.

🧱 **#1005 era el que rompía gates ajenos:** `applySchema` no escribía `schema_migrations`, así que el migrador re-aplicaba la 0028 —CHECK con 12 valores, sin los 2 de la 0031— y tumbaba **el gate del dashboard entero**, con un síntoma que apunta al diff de quien pase por ahí. → [[un-bootstrap-que-aplica-el-estado-final-sin-registrar-los-pasos-hace-que-el-migrador-los-repita]]

🔒 **#981 cerrado con lo que de verdad lo cierra**: la **invariante cruzada** derivada del registro real (los tests caso a caso no cazan al executor que aún no existe) y el CHECK contra Postgres real. Se validó solo al día siguiente: añadir `consultant` a la unión **no compiló** hasta declararlo en el `.pg`.

❌ **Tres errores míos de método, los tres escritos:** `Closes #1007 (3 de 4)` **cerró el issue entero** → [[closes-N-cierra-el-issue-entero-aunque-escribas-3-de-4-al-lado]] · de cuatro mutaciones sin víctima **tres eran el arnés** → [[verificar-que-un-test-tiene-dientes-con-una-mutacion]] · y tres agentes seguidos afirmaron algo falso por leer un checkout **cuatro merges por detrás**.

🧪 **Y una trampa de candado nueva:** con variedad determinista, un caso ejercita SIEMPRE la misma variante, así que aseverar la línea renderizada es ciego a las demás → [[si-la-variante-se-elige-por-el-contenido-el-candado-por-linea-es-ciego-a-las-demas]]
...
- 🔴 **Prod SIN verificar por CONTENIDO tras DIEZ merges de `src/`** — `/ready` 200 es el falso positivo de #780. Faltan: la tarea dictada **en la ficha** (#973) · `SELECT entity FROM audit_log WHERE entity IN ('reminder','open_thread')` con fecha de hoy (#974) · `SELECT sweeper, last_swept_at FROM sweeper_heartbeats` (#953). **No se pueden hacer desde la máquina de Manu**: 5251 y 3000 dan timeout, también fuera del sandbox.
- **Vivas y con dueño:** **#1007** (`pref.set` — reabierto; no es mecánico: una preferencia no tiene fila con id propio, hay que decidir su `entity_id`, y sí necesita migración) · **#1015** (los otros dos leads con ventana reformulable) · **#1009** (el par de claves de Langfuse: sin él, «¿está el presenter encendido y mudo?» es opinión) · **#1002** (suelo real de los 7 ficheros de eval: cuesta una corrida) · #980 · #982 · #983 · #984 · #985.
