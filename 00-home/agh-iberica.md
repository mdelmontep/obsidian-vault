...
## Estado (2026-08-14 noche) — `main` en `52a7c1f`; **CERO PRs mías abiertas** (17 dentro en el día)

🟢 **Dentro el 14-ago:** siete por la mañana, cuatro por la tarde, cinco en la tarde-2 y **seis de noche**: #1190/#1183+#809 (el sello dice la base REAL con pila; y fuera el warning de lint) · #1187/#1178 (la prosa de comentario CSS deja de contar como mutable) · #1194/#1176 (candado del export fantasma) · #1198/#1173+#1174 (`tone.ts`) · #1199/#1171 (la imagen deja de copiar `dashboard/`, −34,8 % de la capa) · #1202/#1179 (el punto ciego de las at-rules). Gate final sobre la **combinación**: `agente 3279/239/5f · dashboard 1223/0/0f · base 52a7c1f` ✓.

✅ **Prod verificada por contenido, tres veces y sin SSH** (`curl …/version` vs `npm run build:stamp -- --print` en árbol limpio). → [[sellar-la-imagen-en-el-build-para-saber-que-corre-en-prod-sin-shell]] ⚠️ **#1199 mueve el sello** (toca `src/version/build-stamp.ts`): tras su deploy, `/version` dará otro valor y eso NO es un deploy fallido.

🔴 **El SSH del host NO está caído** — `nc -z 185.99.186.138 5251` → *succeeded* (443 también), medido dos veces. La premisa «5251 timeout» se venía heredando desde el 14-ago por la mañana. Sin probar la **autenticación**, solo la conectividad.

🔑 **Lo reutilizable del día:** *(a)* **11 de 16 issues con la premisa falsa, y el sesgo NO es constante** — unas se quedan cortas (contaban 15 donde hay 17; 470 líneas donde hay 947) y otras se pasan (un caso citado como «cinco semanas» eran **8 días**, y esa cifra la escribí yo y viajó a Slack, al snapshot y al issue). 👉 *la cifra que nadie midió está mal en la dirección que le convenga al relato*, y **la recomendación de un issue es la premisa que menos se cuestiona**. *(b)* **el hueco está en el CABLEADO, no en la función pura** — sexta vez en la semana: los 6 huecos reales de #1176 eran `main()`, el guard del entry point y los contadores → [[el-hueco-esta-en-el-cableado-no-en-la-funcion-pura]]. *(c)* **antes de elegir «N candados», mide cuántos pueden fallar**: 141 aserciones y solo 4 con variante → la clase se cierra en el instrumento → [[mide-cuantos-pueden-fallar-antes-de-elegir-entre-n-candados-y-un-tripwire]] · [[aseverar-la-igualdad-congela-un-accidente-asevera-el-bicondicional]]. *(d)* **un fichero nuevo es un solo hunk** y el barrido le da un mutante → [[un-fichero-nuevo-es-un-solo-hunk-y-el-barrido-de-mutacion-no-lo-cubre]]. *(e)* **rojos ajenos con cara de reales**: `.dockerignore` ≠ `.gitignore` → [[dockerignore-no-es-gitignore-y-la-basura-local-pone-el-gate-rojo]]. *(f)* **el `$?` que lees no es el del comando** → [[el-exit-code-que-lees-no-es-el-del-comando-que-te-importa]].
...
📋 **Cola libre (12 nuevas del 14-ago noche, `ready-for-agent`):** arnés/proceso #1188 · #1189 · #1192 · #1195 · #1200 · #1201 — deuda #1185 · #1186 · #1191 · #1193 · #1197 · #1182. Las dos que más rinden: **#1200** (`dashboard/test`, 65 ficheros, **sin ningún linter ni la regla de 0 `any`**) y **#1192/#1201** (un fichero nuevo o un diff solo-tests no recibe barrido). **Decisiones mías:** **#1196** (¿`formal` puede leer su plantilla de texto en voz? `cercano` lo tiene prohibido desde #202) · #1167 · #1129 · **#1180 ya decidida (NO entra), no re-litigar**.
