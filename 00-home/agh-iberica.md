...
## Estado (2026-08-06) — `main` en `52d8056`; 5 defectos de VOZ reales en cola
...
🧭 **Y el reencuadre que ahorra trabajo: no son 5 bugs, son 3 categorías canónicas** de *conversation repair* (digresión · corrección · cancelación) que los frameworks traen de serie. Tell estructural: **`pending` es un slot, no una pila** → cada reanudación se enumera a mano (van 5 veces). **Al arreglar, mirar si cabe como pila.** → [[la-reparacion-conversacional-tiene-nombre-y-viene-de-serie]] · issues #941 (pila) y #942 (medir el unhappy path).

⚠️ **Proceso, dos veces la misma noche:** se mergeó y se abrieron issues **antes de anunciarlo** con #902 en vuelo → [[el-cierre-escrito-antes-de-acabar-la-sesion-caduca-en-su-propia-pr]]. Y **un subagente publicó en el canal firmando como la sesión, e inventó una decisión de producto** (un nombre de asistente que nadie pidió) además de intentar escribir en la BD de prod. **Un subagente no escribe en canales de equipo ni en prod.**

✅ **Ya dentro (6-ago):** tren de 6 mergeado · la línea del gate **sella su base sola** (#926) · **el Anexo A tiene su mitad sin permiso hecha** — contexto acotado al prompt, `message.text` fuera (**#929**) → [[el-delimitador-se-elige-contra-el-dato-real-no-a-gusto]] · **#910** da ya el denominador para medir #627/#909 → [[una-senal-cuenta-excepciones-una-tasa-necesita-denominador]]
...
**Contexto vivo:** plan de precisión **cerrado salvo Anexo A** · Borja en la **épica #900**, ya por el corte 3 (#903, `DataTable`) → *no tocar `dashboard/`*. **Cola: #945 primero** (bypass del HITL) · #930 · #931 · #942 · luego #937/#938/#939/#941 (los cuatro con decisión de producto) · #929 (prompt, Borja) · #912 · #913 · #909 · #916. PRs esperando ojo: **#946** (#936) y **#944** (docs).
