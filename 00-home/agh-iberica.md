...
## Estado (2026-08-06) — `main` en `52d8056`; la única función proactiva NUNCA ha funcionado + 5 defectos de VOZ

🔴 **El digest semanal no ha entregado un mensaje JAMÁS, y salió de una consulta que nadie había hecho** (06-ago, análisis read-only). `open_threads` en prod = **3 filas, las tres en `last_delivery_status='error'`, cero entregadas**. Dos causas apiladas: **#952** la plantilla `hilos_pendientes` **no existe en Meta** (`132001`; en prod no hay definida ninguna var de `TEMPLATE`, así que manda el default de `module.ts:37`) — el control que descarta credencial/sender/dispatcher es que los **recordatorios sí entregan** (16 `sent`) por el mismo camino; y **#953** el barrido **lleva 4 días sin correr** (`setInterval(24h)` sin pasada en el arranque, con redespliegue en cada push). ⚠️ **En ese orden o no se nota nada.** #953 lo cogió una sesión paralela (`wt-953`); **#952 = dar de alta la plantilla** (UTILITY/es, un `{{1}}`, copiando `recordatorio_tarea`) — scriptable con el token de prod, que ya tiene `whatsapp_business_management`, pero **con supervisión: es la cuenta Meta del cliente**. → [[persistir-el-error-no-basta-si-ninguna-superficie-lo-lee]] · [[un-timer-mas-largo-que-la-cadencia-de-despliegue-no-corre-nunca]]
...
🔴 **Auditoría de 2 llamadas reales (05-ago) → cinco defectos, y el HITL se puede saltar.** **#945** es el grave: un **«¿Vale?» EJECUTA el lote** (`normBare` borra `¿?` antes de consultar `BARE_CONFIRMS`) — los otros pierden dato dictado, este **escribe en el CRM sin confirmación**. Los demás: **#937** («No, está bien» ejecutado como cancelar, contacto+teléfono perdidos) · **#938** (tras el cancel, «lo que te he dicho antes» no recupera nada) · **#939** (el compound «sí + resto» exige conector, y el copy promete un «seguimos con lo nuevo» que nada guarda) · **#940** (el saludo «Soy Aquí está»).

🧭 **No son 5 bugs, son 3 categorías canónicas** de *conversation repair*; el tell es que **`pending` es un slot, no una pila** (van 5 reanudaciones enumeradas a mano). **Al arreglar, mirar si cabe como pila** — #941, y #942 para medir el unhappy path. → [[la-reparacion-conversacional-tiene-nombre-y-viene-de-serie]]
...
✅ **Ya dentro (6-ago):** tren de 6 · la línea del gate sella su base (#926) · contexto acotado al prompt, `message.text` fuera (#929) · **#910** da el denominador para medir #627/#909 → [[el-delimitador-se-elige-contra-el-dato-real-no-a-gusto]] · [[una-senal-cuenta-excepciones-una-tasa-necesita-denominador]]
...
**Contexto vivo:** plan de precisión **cerrado salvo Anexo A** · Borja en la **épica #900**, corte 3 (#903, `DataTable`) → *no tocar `dashboard/`*. **Cola: #945 primero** (bypass del HITL) · #952/#953 (el digest, ya con dueño el segundo) · #931 · #942 · luego #937/#938/#939/#941 (decisión de producto) · #929 (prompt, Borja) · #912 · #913 · #909 · #916. PRs esperando ojo: **#959** (docs de cierre), #946, #944.
