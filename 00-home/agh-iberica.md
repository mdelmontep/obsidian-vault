...
**Meta #1691:** contrato + anexo + SNC + CV → propuesta humana → ficha cifrada. Tablero del piloto: **53/100**.
...
- 🔴 **Lo vivo es UNA barrera compartida, no dos features: #1931.** `src/http/hr-routes.ts` da **409** a todo `docKind` que no sea contrato y es la ÚNICA ruta que sirve la propuesta (el dashboard proxya): el arreglo de #1908 es correcto y medido contra la tabla real, pero **inalcanzable por HTTP** — ahí sigue cortado CV → ficha. **#1925** (la nómina, que escribe saltándose `hr_field_proposals`) choca contra esa misma puerta. Suelto: **#1929**, `hr_settings` en el CHECK desde la 0044 sin un solo escritor.
- 🔴 **Siguiente:** **#1849** (los tres candados de producción; runbook de 7 pasos con prueba de restauración **antes** del primer documento real) → **#1701** (aprobar y escribir la ficha: el DECIDIR) → #1705 → #1710. De terceros: cuántos contratos de cada familia y los SNC por correo (#1819). Casar documento↔consultor: **#1867**. **#1774 sigue sin eval pagada: no afirmar exactitud, coste ni latencia.**
