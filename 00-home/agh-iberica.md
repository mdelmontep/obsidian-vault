...
## Estado (2026-08-06, tarde) — `main` en `76b31e8`; el bypass del HITL y el sweeper YA DENTRO; la plantilla de Meta sigue siendo el bloqueante

🔴 **El digest semanal no ha entregado un mensaje JAMÁS** (06-ago). `open_threads` = 3 filas, las tres en `error`, cero entregadas. Dos causas apiladas y **en este orden o no se nota nada**: ✅ **#953 dentro** (PR #960, `65c6504`: barrido en el arranque + heartbeat `sweeper_heartbeats`, mig. 0030) — su efecto esperado es que `last_error_at` **empiece a moverse** en vez de estar congelado desde el 2-ago, que es progreso; y 🔴 **#952 VIVO, y es humano**: la plantilla `hilos_pendientes` no existe en Meta (`132001`). Darla de alta (UTILITY/es, un `{{1}}`, copiando `recordatorio_tarea`) es scriptable con el token de prod, **pero con supervisión: es la cuenta Meta del cliente**. El control que descarta credencial/sender/dispatcher: los **recordatorios sí entregan** (16 `sent`) por el mismo camino → [[persistir-el-error-no-basta-si-ninguna-superficie-lo-lee]] · [[un-timer-mas-largo-que-la-cadencia-de-despliegue-no-corre-nunca]]

🥊 **Los dos ADR rechazados, revisados con fiscal y defensa (06-ago).** **ADR-0004** aguanta, pero no por polling-vs-cola: **por la reconciliación en el arranque**. **ADR-0003** (Mem0) sigue bien decidido y **mal vigilado** → **#954**: su criterio de revisión no se ha corrido en un mes, un argumento caducó (el texto del usuario ya viaja al modelo y a las trazas) y el trigger es más estrecho de lo que dice — «recuérdame llamar a X el jueves» nunca es un hilo.

📌 **El hueco de memoria, acotado:** preferencias = **2 claves** que además nunca llegan al intérprete. **#955** `client.recurrent` en el CHECK desde la 0016 y ausente en las 3 capas (rec: quitarla sin consumidor) · **#957** cero noción de prioridad en el dominio, con las 4 señales ya escritas y solo legibles bajo pregunta (medir antes de código) · **#958** (RGPD) sin borrado por sujeto → **con Carlos, junto a la DPA**, como #917.

✅ **El bypass del HITL, CERRADO** (#945 → PR #947, `76b31e8`). El issue pedía el guard «antes de consultar los sets» y ahí cubría **una de cuatro** puertas; va en la costura donde se DECIDE el atajo → [[el-guard-va-en-la-transicion-unica-que-comparten-todos-los-caminos]]. 🔴 **Quedan los 4 que PIERDEN dato dictado**: #937 (un «No, está bien» ejecutado como cancelar) · #938 (tras el cancel, el lote es irrecuperable) · #939 (el compound exige conector, y el copy promete guardar lo nuevo y no lo guarda) · #940 (el saludo «Soy Aquí está», un `UPDATE` de una fila).

🧭 **No son 4 bugs, son 3 categorías canónicas de *conversation repair*** y el tell es que **`pending` es un slot, no una pila** (#941, con decisión de producto dentro) → [[la-reparacion-conversacional-tiene-nombre-y-viene-de-serie]] · guía en `docs/architecture/pragmatica-conversacional.md`.

⚠️ **Proceso (5/6-ago, tres veces ya):** avisos que se cruzan por minutos o segundos — mergear/abrir issues antes de anunciar, y dos sesiones publicando con 3 s de diferencia. Leer antes de decidir no protege del hueco entre *leer* y *publicar*. **Un subagente no escribe en canales de equipo ni en prod.** → [[el-cierre-escrito-antes-de-acabar-la-sesion-caduca-en-su-propia-pr]]

✅ **Ya dentro (6-ago):** dos trenes (6 + 7 PRs). El segundo cerró #930, #931, #936, #945 y #953 — detalle en `docs/status-log/2026-08-06-manu-*` y en `archive-completed`.
...
**Contexto vivo:** plan de precisión **cerrado salvo Anexo A** · Borja en la **épica #900**, cortes 3 y 4 (#903 dentro, #934 dentro, #904 en PR #967) → *no tocar `dashboard/`*. **Tren de 7 merges cerrado el 6-ago** (#969·#944·#956·#946·#961·#960·#947, `main` en `76b31e8`, override de founder documentado): dentro #930, #931, #936, #945 y #953. ⚠️ **Prod SIN verificar por contenido** — el harness bloqueó el SSH; falta comprobar `SELECT sweeper, last_swept_at FROM sweeper_heartbeats` (fecha de HOY) y el guard de #945 en el bundle. **Cola:** #952 (humano, plantilla Meta) · #942 · #937/#938/#939/#941 (decisión de producto) · #966 · #968 · #970 · #929 (prompt, Borja) · #912 · #913 · #909 · #916. Learnings de la tanda: [[rebasar-un-worktree-deja-node-modules-caduco-y-el-gate-no-lo-distingue]] · [[write-tool-byte-nulo-en-template-literal]].
