...
## Estado (2026-08-07, tarde) — `main` sigue en `7b9d270`; **SEIS PRs mías esperando merge** y prod ya verificada

✅ **Prod verificada por CONTENIDO** y las dos comprobaciones de negocio CERRADAS con un turno real de voz: **#973** (la tarea cuelga de la ficha de Dragados, y «última actividad» de Cartera se mueve) y **#974** (`reminder|create` en `audit_log`). El contenedor corre desde `src/` vía tsx, así que el marcador se lee en el TypeScript que ejecuta y **los conteos coinciden con `origin/main`** — no es «aparece», es «aparece las mismas veces».

🔎 **La auditoría del día: los 30 kinds de write, uno a uno** (¿deja fila? ¿deja audit? ¿se ve en el dashboard? ¿se deshace?). Sale que el dashboard es **100 % client-scoped** y TODO lo que no cuelga de un cliente es invisible → **#1031**. **Decidido por Borja: pantalla propia «Lo mío»** (#1033: tareas sin cliente + recordatorios con cancelar + hilos). Fuera y sin decidir: oportunidades y contactos internos.

🔴 **Y un guard de owner que NO existía**: `ReminderStore.cancel` filtraba por `(tenant, id)` con un comentario que decía «GUARD» y un docstring que afirmaba que lo ponía el llamante. No lo ponía nadie — **no tenía llamantes**. → [[un-metodo-sin-llamantes-puede-tener-el-guard-ausente]]

📦 **En PR (ninguna toca `dashboard/web/src`):** #1023/#1015 · #1025/#1007 · #1027/#1002 · #1034/#1028 · #1035/#972 · #1038/#1030-anáfora. **Las cuatro primeras son disjuntas fichero a fichero**, así que se mergean en cualquier orden. #1038 va **sin keyword de cierre**: falta la referencia por cuerpo, que exige prompt y evals.

🧪 **#1002 cerrado midiendo**: los 7 ficheros sin suelo son 35 casos, no una corrida completa (158 llamadas). Y el histórico convertía el ×5 en ×1 → [[repeticiones-adaptativas-dan-menos-muestras-de-las-que-pides]]. El séptimo, `llm-smoke`, **no corre desde que existe** (`LLM_SMOKE=1` no la exporta nadie) → #1026.

⏳ **#1033 espera a #1000**: su alcance da por hecho `.page-header`, que existe en la rama de Borja y **no en `main`** (verificado).

📚 **La madrugada del 7-ago, condensada** (detalle en `docs/status-log/` del repo): tren de **diez** merges de uno en uno con gate re-corrido entre cada uno — **#1003** (el «hecho» de voz decía el infinitivo mientras WhatsApp decía el pasado: la copia existía desde #575 y el camino de voz **nunca la recibía**, por construirse sus líneas aparte) · **#1010** (el presenter podía convertir «3 tareas **vencidas**» en «para hoy» y el guard lo aprobaba → [[ADR-050-el-modelo-frasea-solo-donde-el-guard-puede-rechazarlo]]) · **#1005** (`applySchema` no registraba las migraciones y tumbaba **el gate del dashboard entero**, con síntoma que apunta al diff ajeno → [[un-bootstrap-que-aplica-el-estado-final-sin-registrar-los-pasos-hace-que-el-migrador-los-repita]]) · **#981** (invariante cruzada derivada del registro real; se validó sola al día siguiente: ampliar la unión **no compiló**). Tres errores de método, escritos: `Closes #1007 (3 de 4)` **cerró el issue entero** → [[closes-N-cierra-el-issue-entero-aunque-escribas-3-de-4-al-lado]] · de cuatro mutaciones sin víctima **tres eran el arnés** → [[verificar-que-un-test-tiene-dientes-con-una-mutacion]] · y tres agentes afirmaron algo falso leyendo un checkout cuatro merges por detrás.

⚠️ **Arnés:** Actions sigue sin arrancar por billing (**0 pasos**, medido 7 veces en el día) — y `CLAUDE.md` ya **no afirma** el modo: describe los dos y cómo distinguirlos contando **pasos**, porque también se ha visto `0 pasos` en **15 minutos** con `cancelled`. → [[el-rojo-de-ci-tiene-dos-causas-cuenta-los-pasos-ejecutados]] · el `rebase` local lo bloquea el clasificador del harness: el tren se hizo con `git merge origin/main` por rama.
...
- ✅ **Prod verificada por CONTENIDO el 7-ago** y #973/#974 cerradas con un turno real. El SSH SÍ abre desde la máquina de Manu (5251); lo que fallaba era el clasificador de permisos, no la red. Queda **#953** confirmado por efecto: el barrido de arranque movió `last_error_at` de los 3 hilos tras 4 días congelados, y falla con `132001` porque falta la plantilla de #952.
