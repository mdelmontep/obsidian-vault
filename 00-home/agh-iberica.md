...
- 🔴 **Siguiente (medido 21-sep):** el /goal «Paquita responde por WhatsApp sobre los CV» NO es #1936: son **cuatro cortes** — #1858 (el brain mata el PDF, frontera protegida por test: se cruza con un puerto desde `app.ts`), **el agente no tiene resolutor de permisos** (pieza compartida por #1858/#1859, sin issue propia), de qué consultor es el CV, #1936 y #1859 — más **dos clics humanos** (confirmar CV, aprobar). **#1849 ya NO bloquea**: los dos candados abiertos desde el 18-sep (sonda), y el centinela de clave **ejercido el 21-sep: `ok`** (la clave de prod es la que lo firmó); queda que un documento recorra el camino (`hr_documents=0`). 🔴 **Prod: `LANGFUSE_TRACE_CONTENT=true` y `TRACE_PLAIN_USER_ID=true`** con el defecto del código en OFF — decidir antes de #1859. Casar documento↔consultor: **#1867** (fuera de alcance).
