...
## Estado (2026-08-14 tarde-2) — `main` en `38392c8`; **CERO PRs mías abiertas** (11 dentro en el día)

🟢 **Dentro el 14-ago:** por la mañana siete, por la tarde cuatro (#1162/#1152 · #1163/#1154 · #1164/#1159 · #1169) y por la tarde-2 las **cinco de los agentes**: #1170/#1168 (`.dockerignore` anclado a la raíz) · #1172/#1155 (el golden retrataba **5 de 17** moldes) · #1175/#1165 (la política de `verify-ui.ts` al módulo puro) · #1181/#1166 (la clave de baseline pierde el ordinal de fila) · #1177/#1156. Gate final sobre la **combinación**: `agente 3222/239/5f · dashboard 1213/0/0f`, y `verify:ui` con navegador `0 nuevos · 4 conocidos`.

✅ **Prod verificada por contenido, dos veces y sin SSH** (`curl …/version` vs `npm run build:stamp -- --print` en árbol limpio). La 2ª es mejor evidencia: el digest **cambió solo** y coincidió. → [[sellar-la-imagen-en-el-build-para-saber-que-corre-en-prod-sin-shell]]

🔑 **Lo reutilizable del día:** *(a)* **9 de 12 issues con la premisa falsa en tres días, y el sesgo ya NO es exagerar: se quedan CORTAS** — #1168 apuntaba al fichero equivocado, #1155 contaba 15 moldes donde hay 17, #1165 decía 470 líneas donde hay 947, y **#1166 recomendaba la PEOR de sus tres opciones** (el `rowKey` que proponía como ancla es un UUID que genera la base). 👉 *cuando un issue recomienda una opción, esa recomendación es una premisa más*. *(b)* **un export cuyo único importador es su test sale verde en el arnés, en `tsc` y en eslint** — 5 semanas así → [[un-export-cuyo-unico-importador-es-su-test-sale-verde-en-todo]]. *(c)* **retirar un campo de una interfaz no es un candado**: los spreads esquivan el excess-property check → [[retirar-un-campo-de-una-interfaz-no-es-un-candado-los-spreads-lo-esquivan]]. *(d)* **`mutate:diff` es ciego a los tipos** y también al `.dockerignore` y a los comentarios de bloque CSS (→ #1178) → [[un-arnes-de-mutacion-sobre-vitest-no-ve-los-candados-de-tipos]].

🧰 **En cola para Borja:** **#1097 → #1098, apiladas** — ⚠️ al mergearlas, squashear el padre deja a la hija `CONFLICTING`, **o `MERGEABLE` reaplicando el diff del padre**: `gh pr edit --base` no arregla la historia, hace falta `git rebase --onto origin/main <rama-padre>` → [[delete-branch-al-mergear-cierra-la-pr-apilada-no-la-reapunta]]. Desbloquean **#1144 · #1146 · #1161** (las tres en `hitl-brain.ts`). Además **#1126** (una PR apilada no genera run de Actions) · **#1129** (el `git-guard` del repo deja pasar `git reset -q --hard`). ⚠️ **#1036/#1037 caen en `static.ts` con la #1140 de Dani: apilar, no paralelizar.**

📋 **Cola de agente libre (todas mías del 14-ago):** #1178 · #1176 · #1183 (arnés y gate; ⚠️ #1183 y #809 chocan en `gate-summary.ts`) · #1179 · #1171 (dashboard) · #1173 · #1174 (⚠️ los dos en `tone.ts`, encadenar) · #1182 (barrido de tipos). Decisiones mías: #1167 · #1129 · **#1180 ya decidida (NO entra), no re-litigar**.
...
- **13/14-ago (Manu; 21 PRs en dos días)** — el 13 cinco (el arnés se mide a sí mismo) y el 14 dieciséis, todas de arnés y medición: golden de copia por canal, `verify:ui` que ABRE lo que hay que abrir, el coste de evals en un solo sitio, la línea `pg:`, el barrido cubriendo `dashboard/`, el seed con variantes y el **sello de imagen que verifica prod sin SSH**. **Lo que sobrevive: 9 de 12 issues con la premisa falsa, y el sesgo dejó de ser exagerar — se quedan CORTAS** (una llegó a recomendar la peor de sus tres opciones). Corolarios: *una tanda de mutaciones a mano hereda tu hipótesis, el barrido del diff no*; *`Refs` y nunca `Closes` cuando entra media issue*, comprobado tras el merge; y **dos arneses se validaron solos en su primera PR ajena**. → [[el-hueco-esta-en-el-cableado-no-en-la-funcion-pura]] · [[un-guard-que-decide-por-mencion-bloquea-lo-que-solo-nombra-el-comando-caro]] · [[una-herramienta-que-se-aplica-a-su-propio-fuente-necesita-el-rescate-fuera]] · [[closes-N-cierra-el-issue-entero-aunque-escribas-3-de-4-al-lado]] · [[gh-issue-create-el-titulo-se-rompe-en-silencio-y-los-numeros-no-se-predicen]]
