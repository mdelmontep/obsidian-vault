...
## Estado (2026-08-07, noche) — `main` en `b607a14`; **CINCO PRs mías esperando a Borja**, disjuntas fichero a fichero

📦 **La tanda de la noche.** Borja mergeó **diez** PRs (`7b9d270` → `b607a14`) y **devolvió dos con motivo** (#1023 y #1034), las dos con el gate en VERDE — «el gate verde no es la revisión» es la lección que él mismo escribió, y sus propias #1000 y #1045 llegaron con defectos que la suite daba por buenos. Recogidas las dos, rebasadas y re-gateadas.

**En PR ahora mismo, ninguna colisiona con otra:** `#1023`/#1015 · `#1034`/#1028 · `#1059`/#1057+#1058 · `#1061`/#1039 · `#1062`/#970.

🔬 **Lo que más valió del día no fue un fix, fue el arnés (#1049 → mergeado en #1051).** `npm run mutate:diff` revierte cada cambio de producción de la rama y exige víctima. Corrido sobre la rama de #1023 sin decirle dónde mirar, señala **los dos hallazgos de la revisión humana** —B2 en la línea exacta— **y dos más que ella no vio**. El motivo por el que hacía falta está medido: en #1034 hubo **nueve** mutaciones a mano y el defecto se coló igual, porque las nueve caían sobre los ejes que la PR había diseñado → [[barrer-el-diff-en-vez-de-mutar-a-mano]].

🔴 **`agh_dev` está ENVENENADA e irreparable por `db:migrate`.** Tiene las 33 migraciones REGISTRADAS y el CHECK de `audit_log` con 12 valores; `db:migrate` responde «migraciones aplicadas» y no cambia nada. Causa: el bootstrap las marca **sin ejecutarlas** sobre una base existente, y los ~50 `.pg` lo llaman contra lo que apunte `DATABASE_URL` → **cualquier base compartida se envenena sola**. Salida: `dropdb` + `createdb` + migrar. Guard en #1061; **la raíz (#1060) NO se ha tocado: es el invariante de #1005, decisión de Borja**. → [[registrar-una-migracion-sin-ejecutarla-envenena-la-bd]]
...
🔎 **Auditoría de los 30 writes (cerrada)** — el dashboard es 100 % client-scoped: todo lo que no cuelga de un cliente es invisible (#1031) → pantalla «Lo mío» (#1033, decidida y desbloqueada desde que #1000 está en `main`). Y un guard de owner que no existía en `ReminderStore.cancel`, arreglado en #1038 → [[un-metodo-sin-llamantes-puede-tener-el-guard-ausente]].


🧪 **#1002 cerrado midiendo**: los 7 ficheros sin suelo son 35 casos, no una corrida completa → [[repeticiones-adaptativas-dan-menos-muestras-de-las-que-pides]]. El séptimo, `llm-smoke`, **no corre desde que existe** → #1026.

📚 **Madrugada del 7-ago (todo mergeado)** — tren de diez merges: #1003 (la copia en pasado existía desde #575 y el camino de voz nunca la recibía) · #1010 ([[ADR-050-el-modelo-frasea-solo-donde-el-guard-puede-rechazarlo]]) · #1005 ([[un-bootstrap-que-aplica-el-estado-final-sin-registrar-los-pasos-hace-que-el-migrador-los-repita]] — ojo, es el invariante que #1060 reabre) · #981. Tres errores de método: [[closes-N-cierra-el-issue-entero-aunque-escribas-3-de-4-al-lado]] · de 4 mutaciones sin víctima 3 eran el arnés · y tres agentes afirmaron algo falso leyendo un checkout stale.

⚠️ **Arnés:** Actions **no arrancó en toda la jornada — 9 runs con 0 pasos** en 7 PRs distintas (la causa «billing» es una hipótesis, no está confirmada; lo medido es que no ejecuta). Consecuencia: se mergearon 10 PRs mirando un CI que no corrió los `.pg` — y `CLAUDE.md` ya **no afirma** el modo: describe los dos y cómo distinguirlos contando **pasos**, porque también se ha visto `0 pasos` en **15 minutos** con `cancelled`. → [[el-rojo-de-ci-tiene-dos-causas-cuenta-los-pasos-ejecutados]] · el `rebase` local lo bloquea el clasificador del harness: el tren se hizo con `git merge origin/main` por rama.
