...
🟢 **Seis PRs dentro, siete issues cerrados** (#1303/#1293 · #1287/#1280 · #1311/#980 · #1310/#1290 · #1294/#1260 · #1292/#1282+#1268; último código `4a83b42`). Gate sobre `main` **✓ verde a la primera** (`3670/239/5f · dashboard 1271/0/0f`), prod verificada por contenido, y la **combinación de las siete medida ANTES de mergear** con la suma exacta (3670).

🩺 **El flake `57P01` está cerrado como CLASE**, no por fichero: [[vitest-unhandledrejection-run-rojo-pese-a-0-fallos]] (el `pool.end()` antes del `DROP … WITH (FORCE)` **no basta**, y el swallow por fichero es lo que reincidió tres veces). Y el `tests 0 passed` de la línea del gate tenía root cause: [[la-cabecera-de-error-del-runner-roba-la-primera-ocurrencia-al-parser]] → **#1291** (ya desbloqueado).

🟢 **Langfuse recuperado sin Deploy** (`docker compose up -d clickhouse`, 20 s, los cinco sanos intactos): las **1.064 trazas intactas**, volumen de 37 GB, ventana real **10 días** — el diagnóstico previo y mi propia corrección eran los dos falsos. Receta → [[Stack/docker-infra]]. ⚠️ **Manu confirma que no la usan mucho ahora**: 1.064 trazas en cinco semanas, con días de 4 y 6, así que **cualquier medición de conducta real (auditoría #979, #1064, #535, #591) mide una muestra minúscula** — tenerlo en cuenta antes de gastar en evals. La sonda que lo tapó 10 días → **#1304**.

▶️ **Cola desbloqueada por estos merges**, lo barato primero: **#1305** (una palabra: el subset de #247 es el 4.º sitio que pide la costura de voz) · **#1291** · **#1306** (el backstop del borde solo vigila teléfonos: correo y fecha sin red) · **#1289** (`prepare` fuera de todo `try`) · **#1204** · **#1309** ([[order-by-created-at-empata-dentro-de-la-misma-transaccion]]) · **#1304**. `ready-for-human`: **#1288** (un prefijo derivado del RELOJ pasa los candados de #1260 **y** #999) y **#1307**.

📚 **Estados anteriores** → [[agh-iberica-historico]] (**17-ago mediodía y madrugada** condensados ahí al cerrar la tarde). **16-ago (tarde)**: 8 PRs y 10 issues (`c24d5c9`→`8863b57`) — su lección (`MERGEABLE` no es «el diff es el tuyo») **ya está en la regla 6 de `CLAUDE.md` desde #1272**. **15-ago (noche)**: 13 PRs y 14 issues.
