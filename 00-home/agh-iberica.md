...
## Estado (2026-07-30, mediodía) — el tren de madrugada EN PROD, y 3 PRs nuevas esperando a Borja

**`main` en `7cc0766`** y autodeploy en marcha: Borja mergeó el tren de madrugada (#676/#672 con la **migración 0024**, #677/#674, #678 docs) y su **#679/#661** (modelo de datos de staffing, mig 0025). Issues #672/#674/#661 cerrados. Él sigue con **#662** (permisos con nombre, mig 0026, todo en el dashboard).

**Mías abiertas y rebasadas sobre `7cc0766` con el gate re-corrido** — no le toca rebasar nada: **#680** (#531, `2169/212/3`), **#682** (#681, `2148/203/3`) y **#684** (solo-docs, el cierre). Ninguna toca `SYSTEM_PROMPT`, `schema.sql` ni migraciones.

**El plan de precisión del agente entra al repo** (`docs/specs/2026-07-30-precision-agente.md`): 4 fases + anexo bloqueado, con las alternativas descartadas y su porqué. Tesis en una línea: el agente no interpreta «cualquier petición razonable» porque **no hay bucle agéntico**, **los reads tienen forma de PREGUNTA y no de DATO** (mapa 1:1 → siempre falta uno) y hay más dato escrito que consultable; y por la decisión RGPD, **todo se resuelve en la mitad SIN egress** (el modelo *describe* la consulta, TS ejecuta y formatea).

- **#680 · Fase 0, el termómetro (cero cambio de conducta).** Eje `query` con 9 casos-oro que **nacen ROJOS a propósito** (cruces entre entidades, filtros sobre campos que ningún read acepta, dato escrito sin lectura) y señal `no_target_miss` que instrumenta el fuera-de-superficie. Sin esto ninguna mejora posterior era demostrable. Ojo con dos verdades del gate, ya escritas en `docs/evals.md`: **un eje con `minRate: 0` no gatea la tasa**, solo cobertura (así el rojo esperado no tumba a nadie, pero los «sube de X a Y» son comparación **manual** de scorecards); y **`overall` está desfasado** (suelo de `311/315` sobre ~590 casos) → los gates reales son `byTag`/`byFile`. Su hallazgo de fondo es de RGPD → [[campo-de-texto-libre-que-viaja-a-telemetria-es-un-canal-de-egress]]
- **#682 · precondición P1.** El scoping por `(tenant, owner)` de una lectura de cartera pasa de convenio en un comentario a **error de compilación**; y `ReminderStore.list`, el seam de la Fase 1, **no tenía ni un test contra Postgres**. → [[regla-en-docstring-no-impide-nada-partir-el-interface]] · [[asercion-de-ausencia-necesita-fixture-que-pueda-fallar]]

**Fase 1 desbloqueada y NO arrancada** (espera OK): dos issues y dos PRs — la primera mitad (`RemindersReadTool` + `OpenThreadsReadTool` + filtros de `TasksReadTool`) es autónoma y toca `capabilities.ts`; **la segunda toca `SYSTEM_PROMPT` = carril de Borja** con evals ×3. `tasks.due_date` fuera a propósito (no está en el contrato del agente). Fases 2-3 especificadas, **#602 es la decisión abierta de la Fase 3**, Anexo A bloqueado hasta cerrar la política de egress.

**A humano (Borja):** ojo a **#680/#682/#684** · verificar que el boot aplica 0024+0025 tras el autodeploy · **#675** (RGPD, purga en disco de lotes caducados) · **la opción A de #672** (una línea en `llm-turn-interpreter.ts:324` + evals ×3, lo único que cubre paráfrasis abierta) · **#624 sigue esperando un correo dictado** · #648/#649/#651 y los forks de #580/#627/#585 · **#579 de Dani** · `RETELL_WS_SECRET` sin definir en prod.

**Mío, y necesita creds:** la **corrida real del eje `query`** (`npm run evals -- --only=query-coverage` → los 9 deben salir ROJOS y el eje aparecer en el scorecard, no en `"otros"`). No hay `.env` en esta máquina; sí queda probada sin modelo la aritmética del gate.

_Entorno: el gate corre entero sin Docker con los servicios de Homebrew (5432/6379) — arrancarlos, o los `.pg` se autosaltan y el backstop no se prueba (364 skips vs 203). El drift-gate exige `pg_dump` de la misma major que el servidor →_ [[tests-pg-self-skip-levantar-pgvector-local]]. _Y dos gates seguidos contra el mismo Postgres dan un rojo transitorio._
...
_Método de esta semana:_ [[regla-en-docstring-no-impide-nada-partir-el-interface]] · [[asercion-de-ausencia-necesita-fixture-que-pueda-fallar]] · [[campo-de-texto-libre-que-viaja-a-telemetria-es-un-canal-de-egress]] · [[regiones-distintas-en-el-mismo-fichero-de-test-no-se-afirma-sin-mirar-el-hunk]]
