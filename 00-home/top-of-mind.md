...
- **TuFacturaIA — cola OCR subida masiva EN PROD (PR #802 mergeado)** — arregla el "se sube pero no procesa" (CHECK sin `pendiente` + 200 mentiroso). Mig 449 aplicada + cron `ocr-dispatcher` (`* * * * *`) vivo y verificado. Pendiente: smoke lote grande + **cerrar PR #800 duplicado** (misma feature, otra sesión). [[facturaia]]
