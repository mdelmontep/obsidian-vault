...
- **FacturaIA — `manual-admin.md` actualizar Equipo refundido** — flow nuevo (modal invite + reenvío con cooldown + confirm reactivar propietario-only + menú ⋯). `manual-usuario.md` ya actualizado en PR-3a/PR-3b 2026-05-20
...
- **FacturaIA — workflow n8n `pqSWkDIHqmSVHotB` branch `multi_org_pendiente`** — backend PR-2 #59 listo, workflow no procesa el bucket. Añadir nodo IF tras `resolve-context` que ante `reason='multi_org_pendiente'` envíe lista WhatsApp con `candidate_orgs` → al recibir reply → POST `/api/internal/whatsapp/select-org` → re-invocar resolve. Sin esto el bot multi-org no funciona end-to-end. Telemetría `[whatsapp-resolve] multi_pending` en logs Dokploy lo detecta. Ver [[matriz-permisos-rol-aware-bd-mas-espejo-ts]]
- **FacturaIA — 2 crons nuevos en Dokploy** — `team-expire-invites` (`15 3 * * *`) y `whatsapp-purge-stale-selections` (`45 3 * * *`). Sin esto invitaciones nunca expiran y tabla `whatsapp_org_selection` crece con zombies. Patrón en `docs/architecture/cobros-runbook.md` §"Dokploy cron"
