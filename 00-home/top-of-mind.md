...
- **TuFacturaIA — Módulo Obras (mini-ERP instalaciones, sustituye WAPI) EN CURSO, rama `feat/modulo-obras-wapi` sin mergear** — catálogo + modelo de precio (proveedor×familia, MIN) + presupuestos→aceptación→obra + dashboard/calendario/auditoría HECHOS; **catálogo 745 tipos M.O. sembrado (18-jul)**; deuda diferida app-only cerrada (N+1, dedup, paginación, test paridad); **3ª `/fia-cierre` pasada** (bloqueante seguridad RESUELTO: RPC obras ejecutables por `authenticated`, 4ª reincidencia patrón mig 213, ver [[postgres-revoke-public-no-elimina-grants-individuales]]) + smoke/ux/resiliencia con login real. Pendiente: **merge a main** (QA visual final + `migration repair 471..494`) y **siguiente fase FACTURACIÓN de obra** (Checkpoint B parada humana D1-D4 → issues 008-011; D4 atomicidad resuelto: `createDocument` es TS/no-transaccional → reservar→emitir→confirmar, ver [[consumir-tope-y-emitir-documento-reservar-emitir-confirmar]]). Super-prompt en repo `issues/CONTINUAR-obras-facturacion.md`. Estado durable `issues/CONTINUAR-obras.md`. [[facturaia]]
