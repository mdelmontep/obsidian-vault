...
- **TuFacturaIA — cola OCR subida masiva (PR #802, sin merge)** — 6 commits + hardening; arregla el "se sube pero no procesa" (CHECK sin `pendiente` + 200 mentiroso). Migración renumerada 447. Tras merge: **alta cron `ocr-dispatcher` en Dokploy `* * * * *`** (sin él no drena) + smoke lote grande. [[facturaia]]
