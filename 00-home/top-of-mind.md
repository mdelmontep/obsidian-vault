...
- **AGH Ibérica — agente "Carlos" PROD VIVO · épica CAPACIDAD #118 CERRADA** — 10-jul: espina de contexto conversacional COMPLETA en `main` (L1 ventana de turnos #428 · L2a recentEntities+anáfora a oportunidad #433 · L2b subsume activeClientName, fuente única #444 · anáfora a persona #445). Recall 72%→100%, evals ×3 sin regresiones. Substrato JSONB en `conversation_state` (mig 0012). Decisión clave L2b: dato que sobrevive al write se DERIVA, no se almacena ([[campo-que-sobrevive-a-drop-on-write-derivar-no-almacenar]]). **Próxima sesión: tanda de bugs de voz del drill #192 — ya DIAGNOSTICADOS, prompt de continuación listo:** #403 (lastVoicePointer no persiste en prod, mig 0013, independiente, EMPIEZA AQUÍ) · #436 🔴 (confirm de voz ejecuta propuesta NO OÍDA = viola HITL; coordinar con #425 de Dani, misma región) · #437 (alucina email.send desde «¿lo has hecho?», fix de prompt=lane Borja). **L5/L3-A BLOQUEADOS en decisión RGPD de Borja** (egress de nombres de cuenta al LLM). Correcciones de los agentes: RETELL_API_KEY NO rompe la voz (usa RETELL_WS_SECRET); seeding atascado era el bug #406 (→ PR #426 de Dani). Docker local=colima (pg 5433/redis 6380 docker run). Merge: clasificador exige nombrar el bypass por-PR. [[agh-iberica]]
...
- **TuFacturaIA — RETIRADA DE n8n (WhatsApp) COMPLETA en código — 5 PRs mergeados 2026-07-10, faltan smokes prod** — n8n receptor `pqSWkDIHqmSVHotB` fuera del runtime (active:false, 0 ejec); receptor real = `src/app/api/whatsapp/webhook`. Auditoría destapó 2 regresiones del cutover (arregladas: opt-out STOP #814 · extractos por foto #816+mig451) y 2 flujos huérfanos portados (facturar-movimiento como tool copiloto #820 · cobro-confirmar cliente-final #822) + docs #817. Mig 451 (`bandeja_remitente_wa`) aplicada a prod por MCP (db push bloqueado por red). **Smokes prod (10-jul noche): 1 STOP ✅ · 2 extracto foto ✅** (fix #823 `detail:high` subió 2→8 de 9 movs; falta solo la nómina/único positivo — para 9/9 evaluar gpt-4o) **· 3 facturar-movimiento ❌** (el copiloto NO localiza un movimiento que SÍ existe y está pendiente; falla en `searchMovimientos`, ANTES de `crearDocumentoDesdeMovimiento`; causa a reproducir: filtro importe exacto con signo o `solo_revisar`; params del LLM no quedan en `copiloto_tool_calls`=hash) **· 4 cobro-confirmar ⏳** (necesita teléfono NO-miembro cliente con factura abierta). **2 fixes pendientes**: (a) UX — suprimir "¡Recibido!… en breve lo tienes en la bandeja" cuando el doc resulta ser extracto (confunde: da a entender que acabó antes del "¿Los importo?"); (b) bug búsqueda smoke 3. **Fase 6 (limpieza+archivar workflow) POSPUESTA hasta smokes OK** (n8n=rollback). Detalle: memoria agente `project_facturaia_retirada_n8n`. [[facturaia]]
