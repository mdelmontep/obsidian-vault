...
- **FacturaIA — ampliar regex `redactPII` para CIF español** (`src/lib/conciliacion/ai-enrich.ts`) — regex actual `[XYZxyz]?\d{7,8}[A-Za-z]` exige letra al final, CIF formato `B12345674` (letra inicial + 8 dígitos sin letra final) NO se redacta. Gap detectado al testear PR-A3.1
