...
- **agency-portal PR #55 audit actor passthrough** (2026-05-05) — pasa `actor: { email, name }` desde sesión del portal a `/v1/facturas` y `/v1/presupuestos` para que FacturaIA muestre "Sofía López (sofia@...)" en vez de solo "API: Agency-panel". Tras merge: regenerar `types.gen.ts` (`npx openapi-typescript ...`) para quitar el `as never` workaround. https://github.com/AgentesIA-MAdrid/agency-portal/pull/55
