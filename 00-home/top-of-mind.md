...
- **TuFacturaIA — auditoría de cifrado: Fase 1 (cripto PII/cert .p12/kid/cache-control) + Fase 2 dual-write IBAN en PR #998 (18-jul)** — antes del merge: envs `PII_BANK_ENCRYPTION_KEY`+`PII_BANK_INDEX_KEY` (64-hex distintas) en Dokploy + aplicar mig 510 + `gen:types` + smoke. Gated: A2 (share tokens, decisión producto), A3 (RPCs auth, necesita BD dev), A7/B1/B2/B4/B6. Ver [[cifrado-columna-dual-write-degrada]] · [[blind-index-scoped-por-tenant]]. Detalle en hub «Seguridad». [[facturaia]]
