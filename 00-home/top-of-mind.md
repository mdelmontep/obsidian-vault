...
- **TuFacturaIA — reorg split de ficheros (Tier A/B/C)** — 12 PRs mergeados 2026-07-05 (`--admin`): piloto calcular + convención + 5 settings + 5 vistas (split verbatim + checker IDÉNTICO). Pendiente del split: 3 Tier B + 7 Tier A monstruos + 4 rutas Tier C. **Fixes de NOTES.md (139 hallazgos) CERRADOS 2026-07-05**: 16 PRs MERGEADOS (`--admin`): 11 bugs #713-728 + 4 dead code #729-732 + #747 panel docs detalle (tabla+IVA, bug de layout de QA). QA localhost OK. Review independiente cazó #713 commiteado sin el fix (ver [[git-checkout-file-en-red-check-deja-commit-sin-fix]]). Dup idénticas (~6) DIFERIDAS a post-reorg. Cero bug fiscal en generar-view. Detalle split + prompt de continuación en hub §NEXT. [[facturaia]]
