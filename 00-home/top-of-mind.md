...
- **TuFacturaIA — reorg split de ficheros (Tier A/B/C)** — 12 PRs mergeados 2026-07-05 (`--admin`): piloto calcular + convención + 5 settings + 5 vistas (split verbatim + checker IDÉNTICO). Pendiente: 3 Tier B + 7 Tier A monstruos + 4 rutas Tier C + sesión aparte de fixes de NOTES.md (139 hallazgos). Detalle + prompt de continuación en hub §NEXT. [[facturaia]]
...
- **AGH Ibérica — #136 (#112 calidad conversacional) pendiente REBASE sobre `main` (`e61368e`)** + re-gate (main avanzó con #142/#144 de Borja, tocan `hitl-brain.ts` → conflicto a combinar; **prompt de handoff ya redactado**). En prod ya: #142/#144/#114(#134)/#113/#124/#122. **Testeo intensivo del agente por WhatsApp (bucle dev) → muchos fallos** = confirman que la raíz es la **arquitectura clasificador-1-turno → épica #118** (research profunda verificada; Borja valida el marco: migración por fases preservando HITL determinista + tools; va como **comentario en #118**, NO doc). Issues nuevos del testeo: #155 (duplicados+bucle resolver), #156/#145 (teléfono+contactos), #157 (ficha cliente). **Pendiente mío:** rebase #136 · **deploy Langfuse #82 en casa** (checklist en PR #147) · volcar research a #118. Ver [[agente-accion-disclosure-capacidades-y-asumir-vs-confirmar]] · [[working-tree-en-rama-stale-verifica-antes-de-reimplementar]]. [[agh-iberica]]
