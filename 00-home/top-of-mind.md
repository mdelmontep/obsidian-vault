...
- **TuFacturaIA — reorg split de ficheros (Tier A/B/C)** — 12 PRs mergeados 2026-07-05 (`--admin`): piloto calcular + convención + 5 settings + 5 vistas (split verbatim + checker IDÉNTICO). Pendiente del split: 3 Tier B + 7 Tier A monstruos + 4 rutas Tier C. **Fixes de NOTES.md (139 hallazgos) HECHOS 2026-07-05**: triaje 6 agentes → 15 PRs abiertos (11 bugs con test/repro #713-728 + 4 dead code #729-732), dup idénticas (~6) DIFERIDAS a post-reorg (evitar conflicto con split). Cero bug fiscal en payload de generar-view. Detalle en hub §NOW. Detalle split + prompt de continuación en hub §NEXT. [[facturaia]]
