...
- **TuFacturaIA — cobro Stripe Connect (wa-fase3, #705 en main) — onboarding real BLOQUEADO por KYC + cuenta equivocada** — conciliación validada (localhost/test). Hallazgo 07-07: hay DOS cuentas Stripe — billing usa `acct_1Td5ccGgQMT2aOqB` (sk_live), pero todo el setup Connect/KYC lo hicimos en `acct_1TJrQvQY4tV8FMxQ` (la del CLI). Montar Connect en `acct_1Td5cc`: perfil plataforma + identidad (KYC en revisión) → webhook Connect LIVE + `STRIPE_CONNECT_WEBHOOK_SECRET` Dokploy → activar `cobro_stripe` en org piloto (AgentesiaLab, NO sandbox). Ver [[stripe-cli-cuenta-distinta-de-la-app-price-no-existe-falso-positivo]]. [[facturaia]]
- **TuFacturaIA — PR #777 auditoría billing Stripe (ABIERTO, sin mergear, 07-07)** — 7 fixes TDD (reactivación suspended→active, change-plan por-price, subscription.paused, payment_action_required 3DS, coherencia display↔cobro, mig 436 precio_anual pro/enterprise, comentario). 96 tests+typecheck+lint verdes. **Pendiente: correr `npm run build` en entorno estable** (esta sesión lo mataba a 9min) → verificar mig 436 sin colisión → mergear (--admin) → `db push` a prod. Worktree `.claude/worktrees/audit-billing`. [[facturaia]]
