...
- **AGH Ibérica — agente "Carlos" PROD VIVO · frente de CAPACIDAD avanzando fuerte** — 10-jul (sesión enorme): 8 PRs de capacidad en `main` (#407 medición/scorecard+baseline · #410 misses recall/commitment · #408 reads fijan entidad activa · #409 «¿repite?» · #418 endurecimiento · #420 **diseño doc del substrato conversacional** · #421 **L3-B** síntesis en reads · #422 **L4** prompt: activeClientName + fix clarify-routing). **HITO: recall ~72%→100%, OVERALL evals 99%, cero regresiones.** Infra de medición operativa (`npm run evals:check`, ver [[evals-de-modelo-real-oscilan-agregar-corridas-y-baseline-con-margen]]). **Próxima sesión:** levers restantes de #118 → **L1** (ventana de turnos) + **L2** (recentEntities), que necesitan el **substrato persistido** (JSONB `recentTurns`/`recentEntities` en `conversation_state`, migración **0012**, 0010 reservada a #305); **L5** (cartera) BLOQUEADO en decisión RGPD de egress. Diseño completo en repo `docs/architecture/conversational-context.md`. Norte: natural PERO grounded ([[asistente-enterprise-natural-pero-grounded-no-llm-libre]]). Docker local=colima (pg 5433/redis 6380 vía docker run). Pendiente de Borja (prod): `RETELL_API_KEY` vacía + usuarios en `step=seeding` a medias + bug #406. [[agh-iberica]]
...
| AGH Ibérica | [[agh-iberica]] | Agente "Carlos" · **PROD VIVO** · CAPACIDAD: medición+L3-B+L4 en main (recall 72%→100%); próx: L1/L2 (substrato) + L5 (RGPD) |
