...
- **TuFacturaIA — Integración Holded — smoke v1 6/6 verde, GIRO a v2 en curso (otra sesión)** — v1 funciona (smoke real OK, sin ajustes de mapper) pero está archivada por Holded → se reescribe el provider a v2 (Bearer) en una sesión paralela sobre la rama `feat/holded-integration`. Pendiente: decisión v1-ya-vs-v2-ahora (Manu inclinado a v2) + reparto de sesiones A/B/C. PR #787 (v1) compila verde. [[facturaia-holded-integracion]] · [[facturaia]]
