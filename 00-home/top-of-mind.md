...
- **TuFacturaIA — Módulo Obras (mini-ERP instalaciones, sustituye WAPI) EN CURSO, rama `feat/modulo-obras-wapi` sin mergear** — catálogo + modelo de precio + presupuestos→aceptación→obra + dashboard/calendario/auditoría + 745 tipos M.O. HECHOS. **Fase FACTURACIÓN cerrada (18-jul, Checkpoint B D1-D4 aprobado)**: facturar por líneas + certificación (factura serie A vía createDocument, patrón reservar→emitir→confirmar+reconciliador con recuperación-hacia-delante, ver [[consumir-tope-y-emitir-documento-reservar-emitir-confirmar]]), anulación revierte, stock D2 gated, cashflow potencial (D3), PDF+email del presupuesto de obra, IA (insights/generador/copiloto), enlaces cruzados factura↔obra. Migs 495-501 en prod (sin registrar). `/fia-precommit`+`/fia-cierre full` pasados (bloqueante base-vs-total RESUELTO). **Ola avisos W2 aplicada**: IVA seleccionable 21/10/0% (+exención), idempotencia en facturar/certificar, orquestación extraída a lib, fix N+1 modal, tools copiloto facturarObra/certificarObra (015 completo), prefill UO, OpenAPI obras_potencial, detector de deriva (cron+incidencia). **Dogfood E2E autenticado HECHO** (agent-browser, sesión real): facturar A2026-0168→/emitidas→anular→reversión obra (acum 0, deriva 0) — cazó y fijó un bug real `fuente='obras'` (violaba facturas_fuente_check) que ningún test automático vio, ver [[mock-de-createdocument-oculta-violaciones-de-check-del-nucleo-fiscal]]. Artifact de Natalia actualizado (sección facturación). **3 avisos menores /fia-cierre cerrados (18-jul, `e85ebfea`)**: detector deriva acotado (mig 502, `p_org_id` opcional + solo obras tocadas), toast honesto (`confirmacion_fallida`), abono hereda `obra_id` para traza excluyéndolo del sumatorio `facturado_real` (evita doble-resta, ver [[documento-reversor-con-clave-de-agrupacion-doble-resta-en-agregado]]). **FASE 2 COMPLETA (18-jul)**: pedidos de compra + recepción/albaranes (reusa stock D2) + instaladores/partes (coste real MO) + textos tipo + informes + pulido facturación (%, MAT/MO, certificaciones), en rama `feat/obras-f2-int`, migs 503-509 en Sandbox, `/fia-cierre` 0-bloqueantes (9 avisos corregidos) + QA claro/oscuro + smoke + artifact [404caab9]. **Pendiente: merge a main** (QA visual OK + `migration repair 471..509` + renumerar colisión 495) y **NEXT con Natalia**: flujo corrección descuento familia desde factura. Doc `docs/architecture/obras/fase2-plan-decisiones.md`. Super-prompt `issues/CONTINUAR-obras-facturacion.md`. **Kit de listas (18-jul):** filtros + mostrar/ocultar columnas + orden por cabecera + selección múltiple/acciones en lote en las 8 pestañas + botón Importar tarifa (`/fia-cierre` 0 bloqueantes); smoke en vivo en hub, ver [[setstate-anidado-en-updater-rompe-bajo-strictmode]]. [[facturaia]]
