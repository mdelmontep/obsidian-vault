...
- **AGH Ibérica — agente "Carlos" PROD VIVO · 14-jul: audit del agente (#485) + self-recipient (#482-p1) en prod** — cerrados hoy a `main`: **#485/#501** (`365c1e9`, el agente = 2º escritor de `audit_log`, cada write CRM en la misma tx que la entidad, procedencia en el `after`, sin columna nueva; 4 pg-real de atomicidad incl. rollback; patrón [[audit-log-multi-escritor-procedencia-en-after-before-sin-carrera]]) y **#482-p1/#503** (`0c40fdf`, «mándamelo a mí» → resuelve `users.email` del token de Entra/alta; guard que no secuestra a terceros). #482 estaba CLOSED sin la parte 1 hecha → ahora hecha de verdad. Con esto la secretaria queda **completa en sus dos gaps**. **Follow-ups (no bloquean):** #500 (Borja — visualizar el audit en el dashboard) · backfill opcional de `users.email` para los ya-conectados (Manu) · tramo final de #454 (switch de `routeTurn`, ventana propia) · PR #504 (solo-docs de cierre, a merge de Borja). Gotcha de la sesión: [[subagente-feature-sin-cablear-composicion]]. L5/L3-A bloqueados RGPD. [[agh-iberica]]
...
| AGH Ibérica | [[agh-iberica]] | Agente "Carlos" · **PROD VIVO** · secretaria (P1/P2/P5) + #451 `ClientIntake` + #485 (audit del agente) + #482-p1 (self-recipient) en prod; follow-ups: #500 (Borja), backfill `users.email`, #454; L5/L3-A bloqueados RGPD |
