...
- **AGH Ibérica — agente "Carlos" PROD VIVO · 14-jul: audit del agente (#485) + self-recipient (#482-p1) en prod** — cerrados hoy a `main`: **#485/#501** (`365c1e9`, el agente = 2º escritor de `audit_log`, cada write CRM en la misma tx que la entidad, procedencia en el `after`, sin columna nueva; 4 pg-real de atomicidad incl. rollback; patrón [[audit-log-multi-escritor-procedencia-en-after-before-sin-carrera]]) y **#482-p1/#503** (`0c40fdf`, «mándamelo a mí» → resuelve `users.email` del token de Entra/alta; guard que no secuestra a terceros). #482 estaba CLOSED sin la parte 1 hecha → ahora hecha de verdad. Además **#452/#509** (`4be24b9`, módulo temporal único — dedup de `parseWhen`/`formatLocal`/tz en `when.ts` + `resolveTemporal`, move-don't-reshape, +21 tests DST) y **#508** (fix del git-guard: ignora contenido entre comillas → no más falsos positivos en `gh pr create`/`git commit -m`; mismo fix al hook global). **Follow-ups (no bloquean):** #500 (Borja — visualizar el audit en el dashboard) · #454 (tramo final de arquitectura, `hitl-brain.ts` estado/presentación — lane de Borja) · smoke conductual E2E del self-recipient (dictar «mándamelo a mí» por WhatsApp/voz) · carlos+6 usuarios test sin `users.email` (reconexión M365). Gotchas de la sesión: [[subagente-feature-sin-cablear-composicion]] · regla nueva [[agh-manu-self-merge-own-lane]]. L5/L3-A bloqueados RGPD. [[agh-iberica]]
...
- **TuFacturaIA — vista móvil nativa (Home + tabs)** — alcance decidido, sin código; prompt de plan para Fable listo, lanzar en otra sesión. Spec [[facturaia-vista-movil-nativa]]. [[facturaia]]
