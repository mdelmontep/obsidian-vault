...
- **TuFacturaIA — Obras FASE 4 (cierre gap WAPI) COMPLETA en rama `feat/obras-f4-int`, PENDIENTE OK+merge (19-jul)** — 11 issues (obras-040..050): G2 estadísticas en vivo (parrilla Estado Obras + evolución), G1 almacén ligero (salidas+stock+ubicaciones, default `al_facturar`, N1 resuelto), G3 incidencias/festivos, G4 conciliación pedido↔factura línea a línea, G5 importador Telematel masivo con diff+runs, G7 reabrir obra/fecha finalización/avisos. Migs **513-517 aplicadas a Sandbox SIN registrar**. `/fia-cierre` (12 dim) cazó 1 bloqueante (parrilla escondía obra abierta hoy: TIMESTAMPTZ vs date-only, [[comparar-fecha-date-string-vs-timestamp-iso-como-string-falla]]) → fix+test; QA visual claro/oscuro + smoke 6 caminos; 3 defectos UI corregidos. Artifact https://claude.ai/code/artifact/916dc848-d8c9-4740-9fae-bc3f83362f7b. ⚠️ **MERGE NO TRIVIAL**: la rama salió de base anterior a #1004-1006; main YA tiene `513_obras_presupuesto_total_batch` y #1005 alineó la UI de obras al kit (solapa con mi fix DatePicker). Al merge: rebasar sobre origin/main + resolver conflictos UI obras + renumerar migs 513-517→514+ (o tras cifrado 513-515) + `migration repair`. Diferido: G6 liquidación = solo ADR-005 (N3); Telematel API = Plan B (N5). Estado durable `issues/CONTINUAR-obras-fase4.md`. [[facturaia]]
