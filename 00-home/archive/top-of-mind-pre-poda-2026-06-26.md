...
- **TuFacturaIA — reempaquetado Fase 2B: residual** — #513 + **#515 (UsageProvider aviso 80%) ✅ en main**; price Starter 14€ Stripe/Dokploy ✅; migs 399 (tier Plus + grandfathering) / 402 (series canónicas) aplicadas a prod. Falta: **crear price IDs de Plus 29/278,40 en Stripe live + rotar env `STRIPE_PRICE_ID_PLUS_*`** + smoke prod #513/#515 + QA visual + opcional trial `base-checkout`. Spec [[facturaia-reempaquetado-planes]] · [[stripe-price-id-en-env-vs-precio-bd-editable-derivan]]
- **TuFacturaIA — SEPA cobros/pagos/recurrentes ✅ en prod** (#486-493, migs 386/389-392; B2B, pain.001 pago proveedores, devoluciones, atómico, hardening). Pendiente: (a) golden-file `pain.002` REAL BBVA (Dani) → `tests/fixtures/sepa/retorno-bbva-real.xml`; (b) validación Dani `modo='auto'` recurrentes. `issues/prd-sepa-paridad-holded.md` · Ver [[sepa-pain008-remesa-adeudo]]
...
- **TuFacturaIA — Stock plegado en Pro (mig 399)** — salió de `beta` y se incluye en Profesional (ya NO add-on 12,90€). Pendiente: smoke polish #475 (badge coste/ud + "Por cajas") + confirmar gating en prod tras deploy #509. Ver [[facturaia-modulo-stock]] · [[facturaia-reempaquetado-planes]]
...
- **TuFacturaIA — Stripe add-ons** — reempaquetado (mig 399) plegó Conciliación/Anti-fraude/Stock en tiers (ya NO add-ons). Solo fiscal sobrevive como add-on y `?addon=fiscal` ya dispara checkout real (PR #509). Seguridad ✅ (toggle 402 fuera de plan). Ver [[endpoint-toggle-feature-debe-gatear-enable-por-plan-o-compra]] · [[facturaia-reempaquetado-planes]]
...
- **TuFacturaIA — fichero 303 → sandbox AEAT Pre303 (HITL)** — descargar fichero oficial → importar en Pre303. (Bug casillas L3 corregido #503: cuotas devengadas en 03/06/09 + labels regeneradas; 0 exposición real.) Ver [[consumidor-lee-claves-que-productor-no-emite]]
