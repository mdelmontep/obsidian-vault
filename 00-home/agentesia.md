...
- **`/agency/time`** — dashboard de horas del equipo (Dani/Borja/Manu): ingesta vía hooks locales de Claude Code (`ops/claude-time-tracker`), límites de uso 5h/semanal (+ countdown de reset real, 25-jul, ver hito 0), síntesis IA (gpt-4o-mini) de qué está haciendo cada uno en "Trabajando ahora" (árbol persona→sesión, agrupado por `claudeSessionId` desde PR #166 — antes agrupaba por proyecto y fusionaba sesiones paralelas del mismo proyecto en una sola fila; la fusión de DURACIÓN sigue siendo por sesión, no se pierde). Gráficas "Trabajo del mes" (técnico/proyecto) rediseñadas 25-jul: barras (no líneas) para todas las métricas, panel lateral con total REAL del periodo (no una suma que crece sin techo), 6 rangos (Hoy/Ayer/7 días/Este mes/30 días/Personalizado) × vela 5min-7días — ver hito 0.
- **Chatbot ticketing** desplegado en `89B9QN23hOHDq6oP` n8n
- **Slack workspace** activo, canvases por proyecto

## Próximos hitos

0. **agency-portal — SIN COMMITEAR (25-jul), pendiente PR + migración + rollout al equipo** — sesión larga en `main` local, todo verde (lint+typecheck+test+build, 708 tests) y verificado en navegador contra prod real, pero nada en `origin/main` todavía. Trae: countdown de reset real 5h/semanal (bridge `statusline-rate-limits-bridge.sh` + migración `20260724120000_member_rate_limits_resets_at.sql`, pendiente de aplicar) + rediseño completo de "Trabajo del mes" (barras, panel de totales real sin doble conteo — ver [[sumar-conteos-por-bucket-sobrecuenta-vs-total-real-del-periodo]] —, 6 rangos × vela 5min-7días). Pendiente: (a) commit+PR, (b) que Borja aplique la migración al mergear, (c) Borja/Dani/Manuela vuelvan a correr `./install.sh` para activar el countdown (necesita `jq`; el runner headless de Manuela probablemente no lo active, statusLine no se dispara en `claude -p`).
