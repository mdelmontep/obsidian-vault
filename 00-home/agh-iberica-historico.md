...
## 2026-09-20/21 · People & Culture: las cuatro lanes del CV y el cierre de #1931

- ✅ **Cuatro lanes del 20-sep** (#1932 · #1934 · #1930 · #1933, cierre #1935): candado del CHECK de auditoría · claves de CV de punta a punta en el dashboard · `accept` del selector derivado de los MIME legibles · corpus de CV con conteos exactos. **Premisa de #1902 FALSA al medirla** (cero escritores de `consultant_profile`) y aun así había hueco → candado, no cierre en falso. Nota: `status-log/2026-09-20-manu-cv-a-ficha-cuatro-lanes.md`.
- **#1931 (21-sep, PR #1937 `342eb576` + cierre #1938 `8500ceae`).** La pregunta de diseño que el issue dejaba abierta —¿la propuesta de CV por la ruta de siempre o por una propia?— se cerró con **misma ruta y puerta DERIVADA**: `formaDePropuestaServida` lee de `RECLAMOS_DE_WORKER` qué tabla escribe cada worker (sonda sobre el `INSERT` real, nadie la escribe a mano) en vez del literal de `docKind`. `sheet` fuera con motivo duro: serviría `verification: "form_field"` sobre una celda sin bbox. Ocho mutaciones, ocho con víctima.
- **Premisa del encargo FALSA:** «el camino CV → ficha está cortado en un sitio» — estaban cortados **cuatro**; #1931 entrega la superficie de REVISIÓN, no la escritura. La mitad que falta es **#1936**.
- **Hueco destapado y cerrado en la misma PR:** `nombre_completo` es clave de contrato Y de CV → todo CV se abría editable en `HrDecisionReview` y aprobarlo escribía `people.full_name`. Gate `wrong_doc_kind` en `loadState()`, un solo `if` para `preview` y `decide` (E1: la ruta de decisiones **nunca** proxya al agente).
- Hallazgo abierto después del cierre: **#1939**, la misma colisión de clave en el ETIQUETADO. Se había anotado como «a ver si #1936 lo absorbe» y se rectificó: #1936 tarda semanas, así que el hallazgo se quedaba sin cola. **Medido al día siguiente: síntoma FALSO** — los dos literales son idénticos (`"Nombre completo"`), el test habría salido verde sin medir nada; devuelta a `needs-triage` como robustez.
- Método: [[un-cierre-documental-escrito-antes-del-merge-entra-mintiendo]] · [[la-cola-del-gate-miente-en-dos-direcciones]].
