...
- **Dos campos confundibles: pide los dos y crúzalos** — si el LLM puede confundir dos campos del mismo documento (nº factura vs código de cliente, base vs importe neto, emisor vs receptor), no elijas cuál pedir: pide AMBOS y compáralos en código. La colisión es señal determinista de lectura mala. Escribir la regla en el prompt falló dos rondas seguidas sobre el mismo PDF. Ver [[dos-campos-confundibles-pide-los-dos-y-cruzalos-en-codigo]]
...
## Retirados de hot.md el 2026-07-27 (tope de 25)
...
- **`gh pr merge --delete-branch` puede fallar en local (worktree con `main` en otro path) aunque el merge en GitHub YA se hizo** — verificar `state=MERGED` antes de asumir fallo; borrar rama remota/worktree aparte. Ver [[gh-pr-merge-delete-branch-no-borra-la-rama-si-falla-su-checkout-local]].
...
- **Si la config define una escalera ORDENADA, el invariante es el orden, no el min/max de cada campo** — validar campo a campo deja pasar una escalera incoherente. Ver [[config-escalera-ordenada-validar-el-orden-no-solo-min-max]]
- **`smallint` usado como boolean en SQL Server legado es `-1` = true, no `1`** — un ETL que asume 1 invierte la mitad de las filas en silencio. Ver [[sqlserver-smallint-boolean-es-menos-uno]]
- **1Password CLI**: `op item delete` falla en items con campo `ssoLogin` (hay que borrar el campo antes); instalar en Apple Silicon es `darwin_arm64`, no `apple_arm64`; y deduplicar agrupa por título+usuario+password, nunca solo por título. Ver [[op-item-delete-falla-con-campo-ssologin]] · [[op-cli-instalar-arm64-sin-homebrew]] · [[dedup-1password-por-titulo-mas-usuario-no-solo-titulo]]
- **El localhost que mides puede ser otro proyecto** — con varios Next abiertos el dev se va del 3000 sin avisar; el `<title>` lo identifica en una línea. Ver [[puerto-3000-puede-servir-otro-proyecto-verifica-el-title]]
