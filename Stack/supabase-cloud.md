...
- **`db push` avisa `failed to cache migrations catalog … pgdelta-target-ca.crt: ENOENT` y aun así aplica** (CLI v2.109, 24-sep): es la caché de catálogo de pg-delta, no la migración. Confírmalo leyendo `schema_migrations` y la fila tocada, no reintentes.
