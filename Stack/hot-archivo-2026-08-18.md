...
## Movido el 2026-08-25

Gotchas de un stack concreto (su casa es `Stack/<tool>.md`, que ya carga por disparador) que estaban
ocupando sitio en el arranque sin disparador. Wikilinks intactos:
...
- **Al retirar una columna o su escritor, grep de quién la LEE** — el `DROP` no avisa (cuerpo en cadena = sin `pg_depend`, revienta en runtime) y un guard sobre columna ya vacía tiene tests verdes sin proteger nada. Ver [[funcion-sql-con-cuerpo-en-cadena-no-registra-dependencia-de-columna]]
