...
- **Si el contrato ya acepta el campo, el bug está en la pantalla** — y si son tres pantallas, es un supuesto de diseño, no tres bugs. [[el-contrato-acepta-el-campo-y-la-pantalla-escribe-hoy]]
...
- **«✔ verificado contra el repo» encuentra la migración que CREÓ la columna, no la que la borró** — un nombre de esquema se verifica en la ÚLTIMA migración que lo toca. Ver [[verificado-contra-el-repo-no-ve-la-columna-que-un-adr-mando-borrar]]; y contra el REGISTRO tampoco basta — registro y catálogo son dos preguntas: [[una-migracion-aplicada-sin-registrar-hace-mentir-a-migration-list]]
...
- **Un desplegable filtrado por «lo válido» convierte un valor inválido en «ninguno»** [[filtrar-las-opciones-por-un-predicado-convierte-lo-invalido-en-ninguno]]
- **Un 429 guardado como la entidad, con autoguardado, borra el dato real** [[un-error-guardado-como-entidad-con-autoguardado-borra-el-dato-real]]
...
- **Con squash-merge, «¿esta rama está en main?» se pregunta al PR** — `--is-ancestor`, `git cherry` y el diff de tres puntos dan las tres «sin mergear» sobre ramas ya integradas; el test barato es `gh pr list --head <rama> --state all`. Cuatro reincidencias. Ver [[git-merge-base-is-ancestor-falso-negativo-con-squash]] · [[tres-puntos-y-git-cherry-mienten-en-ramas-squash-mergeadas]]
...
- **Antes del primer write real en un sistema ajeno: enumera SUS automatismos, no los de tu orquestador** — y planifica solo hacia delante; un alta con fecha pasada despierta los avisos de esa fecha. Ver [[el-inventario-de-automatismos-no-esta-solo-en-el-orquestador]] · [[dar-de-alta-con-fecha-pasada-despierta-los-automatismos-de-esa-fecha]]
