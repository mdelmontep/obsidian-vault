...
- **"No hardcodear el modo" lo vuelve inverificable desde el repo** — `TRADING_MODE`/`DRY_RUN`/`SANDBOX` viven solo en el panel, así que el checklist da verde leyendo una plantilla. Un modo mal puesto no rompe: ejecuta bien lo irreversible (32 h en `live`, −1.613 USDT). Léelo del sistema vivo. Ver [[no-hardcodear-el-modo-lo-hace-inverificable-desde-el-repo]]
