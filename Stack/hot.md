...
- **Un comentario que afirma una invariante es una deuda de test** — grepea la afirmación contra el código; si la justificación es más específica que el `if`, falta el `if`. Sobreviven a las revisiones porque el revisor lee la frase y deja de comprobar. **Gemelo**: el que declara algo imposible de arreglar → cuenta las filas antes de creerlo. Ver [[un-comentario-que-afirma-una-invariante-es-una-deuda-de-test]] · [[una-prohibicion-escrita-en-un-comentario-merece-medirse-antes-de-aceptarla]]
...
- **Un checker vale por CÓMO se pone rojo** — el que no puede (crawler que sale 0, guard que nunca viste fallar) y el que se pone rojo por la razón equivocada, peor porque enseña a ignorarlo. Una precondición de entorno falla en el arranque y en claro, no vía asertos. Y antes de culpar al cambio reciente, reproduce contra el commit anterior. Ver [[un-checker-que-se-pone-rojo-por-la-razon-equivocada-es-peor-que-no-tenerlo]]
