...
- **Antes de `git worktree remove --force`, `git -C <wt> status --porcelain` Y `stash list` de cada uno** — sin `git add` no queda blob que recuperar (tres tracks de agentes perdidos así, 7-ago), y **`refs/stash` es por worktree**: borrar el worktree se lleva su pila entera (8-ago). Ver [[claude-code-agentes-worktree-failure-modes]] · [[refs-stash-es-por-worktree-y-worktree-remove-se-lo-lleva]]
