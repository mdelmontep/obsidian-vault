...
- **Un comentario que afirma una invariante es una deuda de test, no un guard** — grepea la afirmación contra el código que la implementa; si la justificación es más específica que el `if`, falta el `if`. Sobreviven a varias revisiones porque el revisor lee la frase y deja de comprobar. **Gemelo**: el que declara que algo no se puede arreglar → cuenta las filas antes de creerlo. Ver [[un-comentario-que-afirma-una-invariante-es-una-deuda-de-test]] · [[una-prohibicion-escrita-en-un-comentario-merece-medirse-antes-de-aceptarla]]
...
- **Subagente que reporta «hecho, verde» sin que exista el código** — `git show --stat` + `grep` del símbolo + rojo-primero repetido por ti. Y si le recortaste el gate por carga, su verde no cubre lo recortado: 3 de 5 fixes no compilaban con 8.500 tests verdes. Serializa antes que recortar. Ver [[subagente-reporta-hecho-codigo-que-no-existe-o-no-compila]] · [[gate-parcial-a-subagentes-traslada-el-coste-al-recogedor]]
