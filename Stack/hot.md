...
- **El número de migración libre se mide en PROD, no en el repo** — si ya está aplicado, `db push` se salta tu migración **sin error**. Reincidió 2 veces el 12-ago. Ver [[numero-de-migracion-libre-se-mide-en-prod-no-en-el-repo]]
...
- **`npm ci` va DESPUÉS del rebase, y en cada proyecto del gate** — un worktree rebasado tiene `node_modules` presente pero CADUCO; el gate muere en el typecheck y dice «murió antes de arrancar», que es cierto y no nombra la causa (3 corridas perdidas en un tren de 7 merges; 2 más el 7-ago). Ya hay máquina en camino (AGH #1062) → **sale de aquí cuando entre**. Ver [[rebasar-un-worktree-deja-node-modules-caduco-y-el-gate-no-lo-distingue]]- **"No hardcodear el modo" lo vuelve inverificable desde el repo** — `TRADING_MODE`/`DRY_RUN` viven solo en el panel y el checklist da verde leyendo una plantilla. Un modo mal puesto no rompe: ejecuta bien lo irreversible (32 h en `live`, −1.613 USDT). Léelo del sistema vivo. Ver [[no-hardcodear-el-modo-lo-hace-inverificable-desde-el-repo]]
...
- **Un test no vale hasta que le rompes el código y falla** — miente en las DOS direcciones: un «✓ víctima» **sin recuento de tests** es un mutante que ni compiló. Si sale SIN víctima, sospecha del **arnés** antes que del candado. Diez modos; los últimos: aserción en **rango**, **universo vacío**, **informe que recorta**, el servidor **sirve el código viejo**, **dos puertas** que se cubren entre sí, y la sonda que **retrasa el envío en vez de la entrega** ([[al-provocar-una-carrera-con-page-route-retrasa-la-entrega-no-el-envio]]). Ver [[verificar-que-un-test-tiene-dientes-con-una-mutacion]]
