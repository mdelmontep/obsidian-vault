...
- **n8n langchain toolCode con schema → args en $input.first().json** — `query` queda vacío. Parsear `query` (patrón pre-2.15) devuelve siempre `{}` y validas como si el LLM no hubiera mandado nada. Ver [[n8n-langchain-toolcode-args-en-input-no-en-query]]
