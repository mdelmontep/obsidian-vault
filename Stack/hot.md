...
- **Un test no vale hasta que le rompes el código y falla** — y si la mutación sale SIN víctima, sospecha del **arnés** antes que del candado: medido 3 de cada 4. **Tres modos más (FacturaIA 10-ago): aserción en RANGO que incluye el fallo** (`>=400` daba por protegido un endpoint sin gate, porque sin gate salta el 422 de validación) · **universo vacío** en candados derivados del repo (129 casos «passed» sobre cero elementos) · **el informe ajeno miente** (el JSON de Playwright recortó a 14 anotaciones de 78: la cobertura se mide con marcador propio). Ver [[verificar-que-un-test-tiene-dientes-con-una-mutacion]]. **Y mutar a mano solo cubre tu hipótesis: el diff es la lista objetiva** (una PR con 9 mutaciones se coló igual, todas sobre sus propios ejes) → [[barrer-el-diff-en-vez-de-mutar-a-mano]]
