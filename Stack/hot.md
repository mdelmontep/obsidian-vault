...
- **Un gate solo puede fallar HACIENDO RUIDO** — si hay un camino donde no mide y sale con 0, será el de producción. Al matar un CI, migrar sus gates uno a uno. **Pruébalo desde un cwd que no sea la raíz**: 5 trinquetes con ruta relativa no corrían desde un subdirectorio y salían verdes (9-ago). Ver [[un-script-gate-con-guard-de-entrypoint-degrada-a-no-op-silencioso]] · [[gate-con-ruta-relativa-no-corre-desde-subdirectorio-y-sale-verde]] · [[una-metrica-por-regex-sin-test-del-parser-cuenta-ruido-y-se-vuelve-ignorable]]
...
- **Sacar CSS a un `.module.css` cambia quién es el rival: un empate de especificidad contra `globals.css` lo decide el orden de los chunks, no tú** — anclar a una clase hasheada del componente convierte "gana por orden" en "gana por especificidad". Ver [[empate-de-especificidad-entre-globals-y-un-module-lo-decide-el-orden-de-inyeccion]]
- **Renombrar clases a CSS Modules falla PERDIENDO un estilo sin error** — la red que lo ve es un baseline de estilos computados indexado por RUTA de DOM (la clase es lo que cambia), con skeleton/vacío/error forzados por `page.route`. Ver [[baseline-de-estilos-computados-por-ruta-de-dom-para-migrar-css-sin-e2e]]
