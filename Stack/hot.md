...
- **Tocar un prompt: la posición manda y n=10 no basta** — la regla va PEGADA al dato que prohíbe usar [[dato-en-bloque-de-contexto-se-lee-en-voz-alta-aunque-no-este-en-el-guion]]; dentro de su viñeta se aplica, en una nota posterior no; un literal JSON en un READ sangra al WRITE; comparar variantes exige **n≥25 entrelazado**. Ver [[un-prompt-es-una-superficie-con-localidad-no-un-documento]] · [[una-suite-de-evals-cuesta-llamadas-por-prompt-mide-el-cache-antes-de-proponerlo]]
