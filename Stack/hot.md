...
- **Leer prod sin poder escribir: `BEGIN READ ONLY`** — ni `SET SESSION` ni `options=` funcionan con el pooler; prueba el candado con una escritura REAL, no con `CREATE TEMP TABLE`. Ver [[set-session-read-only-no-persiste-en-el-pooler-usa-begin-read-only]]
...
- **Un test no vale hasta que le rompes el código y falla** — dos minutos distinguen «pasa» de «vigila». Caza el que se salta, el que mide el vecino y el candado que **redondea** su umbral. Ver [[verificar-que-un-test-tiene-dientes-con-una-mutacion]]
...
- **Tercera variante de la misma regla en un detector: para** — enumerar sintaxis se queda corto (S19); comprueba la identidad. Reincide en ESLint mirando solo el objeto de una llamada a método, no el nombre del método. Ver [[un-detector-que-enumera-sintaxis-se-queda-corto-comprueba-la-identidad]] · [[regla-eslint-de-secreto-en-llamada-a-metodo-debe-mirar-tambien-el-nombre-del-metodo]]
...
- **Un lockfile puede pinnar un paquete retirado del registro, y solo lo revela un build con caché limpia** — `npm ci` en el deploy falla, `next build` local sigue funcionando indefinidamente. 17 commits de despliegue caído en silencio. Ver [[lockfile-pinna-paquete-npm-retirado-del-registro-build-limpio-lo-revela]]
...
- **`Workflow({args: [...]})` con un array real puede llegarle al script como string JSON** — si un `Workflow` con `args` array se para con un error de "clave desconocida" sin gastar agentes, prueba con `args` como string plano separado por comas. Ver [[workflow-tool-args-array-llega-como-string-json-pasa-csv-plano]]
- **Un `Artifact` solo lo republica la cuenta que lo publicó** — «could not verify... non-member reader» tras 2-3 reintentos idénticos no es un fallo transitorio: pregunta si se publicó alguna vez desde otra cuenta/sesión antes de seguir reintentando. Diagnóstico rápido: mintar SIN `url` primero — si funciona, el servicio está arriba y es la URL vieja la huérfana. Ver [[artifact-solo-lo-republica-la-cuenta-que-lo-publico]]
- **Un control negativo que no discrimina invalida el test entero** — antes de montar un arnés de verificación (round-trip, sonda, canario), corre primero la versión ROTA. Si también pasa, el instrumento mide otra cosa y el verde es cobertura aparente. Ver [[un-control-negativo-que-no-discrimina-invalida-el-test-entero]]
- **Un modal que reenvía el snapshot completo revalida de balde valores sin tocar** — si el backend valida por delta (clave presente = "se toca"), guardar sin cambiar nada puede rechazar por un valor legacy ya inválido como nuevo. Guardar `valoresIniciales` y mandar solo el delta real. Ver [[modal-que-reenvia-snapshot-completo-revalida-de-balde-valores-sin-tocar]]
