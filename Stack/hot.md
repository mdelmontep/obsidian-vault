...
- **Efecto que recarga por org + setters que mergean prev = estado stale de la org anterior** — reset explícito de todo el estado per-entidad al inicio de `load()`. Ver [[effect-reload-por-entidad-setters-merge-exigen-reset]]
- **Dokploy API `compose.one` devuelve `deployments[]` SIN ordenar** — leer `[0]` como "último deploy" = falso "webhook roto" + deploy manual redundante (caso 2026-06-12). Sort por `createdAt` siempre; las Recent Deliveries de la GitHub App (verde + "Deployed N apps") son la fuente de verdad. Ver [[dokploy-api-deployments-sin-ordenar]]
- **Smoke de RPCs/triggers contra prod sin residuo** — envolver en `BEGIN; DO $$ ...asserts... $$; ROLLBACK` con org/factura temp + user real. Cero teardown. Ver [[smoke-prod-en-transaccion-rollback]]
- **Agregado cacheado sobre ledger compartido (anticipo.importe_aplicado) → recompute por trigger** desde Σ(activas), no a mano en el RPC; si no, otros caminos (desvincular) lo dejan stale. Ver [[agregado-cacheado-sobre-ledger-recompute-trigger]]
