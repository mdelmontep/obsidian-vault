...
- **Antes de `git worktree remove --force`, `git -C <wt> status --porcelain` de cada uno** — sin `git add` no queda blob que recuperar. Tres tracks de agentes perdidos así (7-ago). Ver [[claude-code-agentes-worktree-failure-modes]]
