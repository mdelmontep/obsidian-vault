...
- **Un nodo con lógica correcta pero SIN conexión de salida nunca llega a la respuesta** — se ejecuta, calcula bien, y el resultado se pierde en silencio si nada aguas abajo lo consume. Caso real Simarro: `Build Zones` calculaba perfecto un mensaje con poblaciones cercanas, pero no tenía ninguna arista saliente (`connections['Build Zones'] === {}`) y el nodo que sí respondía al caller (`Format For Voice`, en el workflow WRAPPER, no en el mismo) ni siquiera leía su campo `message`, lo pisaba con un texto genérico fijo — dos fallos independientes en la misma cadena. Al añadir/tocar cualquier nodo de "found:0 → mensaje alternativo", trazar la conexión completa hasta el nodo que responde de verdad, no asumir que "tiene buena lógica" = "se usa".
...
- **Link `maps.app.goo.gl` hardcodeado en plantilla/prompt rompe la vista previa de WhatsApp** (Firebase Dynamic Links apagado ago-2025) aunque resuelva bien en navegador. Sustituir por `https://www.google.com/maps/search/?api=1&query=<lat>,<lng>`. Revisar en cada cliente que comparta ubicación por WhatsApp. Ver [[maps-app-goo-gl-firebase-dynamic-links-rompe-preview-whatsapp]]
- **Reenganche que dispara por "último mensaje fue del bot + pasó tiempo" reabre conversaciones ya cerradas bien** — toda conversación terminada también acaba con el bot hablando último; hay que mirar el contenido del último mensaje del humano (¿fue un cierre corto tipo "gracias"?) para no reabrirla. Ver [[reenganche-por-ultimo-mensaje-del-bot-dispara-tambien-en-conversaciones-bien-cerradas]]
...
- **Qué gate se come los avisos, en un query** — `count(*) filter (where d.data like '%"<Nodo>"%')` sobre `execution_data` join `execution_entity`, un filter por nodo de la cadena: dice cuántas ejecuciones llegaron a cada gate sin abrir ninguna en el UI (10-ago Elphis: 73 llegan a `Preparar notif`, 8 pasan `If notificar`, 3 envían). Filtrar por valores (`%destino":"none%`) falla por el escape del JSON anidado; por nombres de nodo, no. En Postgres `regexp_matches` con `.{N}` revienta por encima de 255.
- **`queryReplacement` manda el TEXTO `"null"` cuando la expresión evalúa a null** — `$N::bigint` casca antes de que el `COALESCE` actúe, y si la columna es `text` el `'null'` se guarda como cadena. Envolver: `COALESCE(NULLIF(NULLIF($N,'null'),'')::bigint, columna)`. Ver [[una-expresion-que-evalua-a-null-viaja-como-el-texto-null]]
- **Inventaría los webhooks vivos antes de dar por bueno un go-live** — `select w.name, x->'parameters'->>'path' from workflow_entity w, jsonb_array_elements(w.nodes::jsonb) x where w.active and x->>'type'='n8n-nodes-base.webhook'`. El campo `authentication` NO basta: varios validan el token en un Code posterior. Caso Elphis: `purge-idem`, sin auth → `TRUNCATE` de dos tablas, vivo desde mayo. Ver [[el-utillaje-de-pruebas-se-queda-encendido-en-produccion]]
- **Un sub-workflow ACTIVO puede no ser llamado por nadie** — `select id,name from workflow_entity where nodes::text like '%<ID_SUB>%'` antes de editarlo; cero filas = huérfano y el arreglo no tiene efecto (10-ago: `notify-cita-confirmada-email` editado con PUT 200 y checks verdes, muerto desde el rediseño de junio). Es la versión «workflow entero» de la línea de arriba sobre ramas. Ver [[workflow-activo-no-significa-llamado-grep-su-id-antes-de-editarlo]]
