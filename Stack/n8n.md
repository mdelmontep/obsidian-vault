...
- **Webhook `status_lead` dispara en TODOS los cambios de estado** — no solo el target. Siempre añadir IF node con `status_id == X` después del webhook para evitar ejecuciones duplicadas (emails, Calendar, tareas). Ver [[kommo-webhook-status-lead-dispara-en-todos-los-cambios]]
- **Custom field names (`field_name`) son texto libre y cambian entre cuentas** — al migrar workflows entre cuentas Kommo, verificar nombres exactos de campos personalizados (ej: "Dia preferencia cita" vs "Día de preferencia", "email" vs "Email"). Obtener con `GET /api/v4/leads?with=contacts` y comparar
