...
- **El "ID" que Kommo enseña bajo «Cuenta de WhatsApp Business» no es el WABA ID — y tampoco tiene por qué ser el `phone_number_id`.** ⚠️ Corregido el 11-sep: en Laserys ese ID (`2603510696778315`) **no correspondía a ningún activo del portfolio**; el WABA era `2639880919800077` y el número `1350959428096173`. **Resuelve siempre el ID contra Meta antes de afirmar qué es**: `business.facebook.com/latest/settings/whatsapp_account?...&selected_asset_id=<ID>&selected_asset_type=whatsapp-business-account`. **Nunca lo verifiques metiéndolo en `business_id=`**: si no corresponde, Meta no da error — hace *fallback silencioso* al portfolio por defecto y parece que el ID pertenece a otro cliente. Ver [[un-id-que-ensena-un-saas-hay-que-resolverlo-contra-la-plataforma-que-lo-emite]].
- **Si el canal pierde el número y Kommo dice que no fueron ellos**: el *Registro de actividad* de la WABA en Meta (`whatsapp_manager/activity_log?business_id=<pf>&asset_id=<waba>`) ubica el cambio. Sin entrada ese día, la desvinculación es interna de Kommo. Ver [[el-log-de-auditoria-de-la-plataforma-duena-dice-de-que-lado-ocurrio-el-cambio]].
...
- **El nodo `n8n-nodes-kommo` con `resource: tasks` IGNORA el filtro `task_type_id`** (medido 12-sep-2026, ejecución 13283: la URL que genera no lo incluye) y devuelve **un item con `_embedded.tasks[]`**, no una tarea por item. Así que `$json.id` sale `undefined` y el nodo siguiente escribe en `/tasks/undefined`: en Simarro el cierre de la tarea de visita al cancelar **nunca había funcionado**, en silencio. Fix: `httpRequest` a `/api/v4/tasks?filter[entity_type]=leads&filter[is_completed]=0&filter[entity_id][0]=<lead>` + un Code que filtre `task_type_id === 2` y ordene por `complete_till`. Relacionado: [[recordatorios-visita-por-task-type]].
...
- **Los disparadores de etapa del Digital Pipeline no salen por la API pública** (22-sep-2026). Solo se ven en el engranaje de la etapa. Antes de crear leads en una etapa, hay que mirar qué ejecuta. Ver [[el-inventario-de-automatismos-no-esta-solo-en-el-orquestador]].
- **La condición de un disparador sobre un campo de fecha pide una fecha concreta.** No hay «tiene valor» ni «no vacío». Para no enviar sin cita, la condición va dentro del salesbot, no en el disparador.
