...
- **Bot WhatsApp sticky multi-org** (mig 125): `whatsapp_org_selection` PK user_id, TTL 20 min sliding (verificado `whatsapp-resolve.ts`, 2026-07-05 — corrige nota previa de 6h/24h stale). Resolver (`resolveOrgContext` en `src/lib/internal/whatsapp-resolve.ts`): N=1 directo · sticky viva · sino `multi_org_pendiente` con candidates (lista interactiva, prefijo `org:`/`org_select:`). **El fallback a `active_org_id` se RETIRÓ 2026-06-01** — no reintroducir, silenciaba la pregunta multi-org. Consumidor real hoy: webhook Next.js `/api/whatsapp/webhook` (imagen/texto), NO n8n (ese workflow es solo voz). Ver [[whatsapp-multi-tenant-matching-debe-ser-por-numero-remitente-no-por-phone-number-id]] (resolución por remitente vs. `phone_number_id`).
- **Cambiar de empresa RECARGA el documento**, no `router.refresh()` (`workspace-switcher.tsx`, PR #1334, 2026-07-29). `refresh()` solo invalida el árbol RSC: los ~106 client components que cargan con `fetch` en un `useEffect` de montaje no se remontan y siguen pintando los datos de la org anterior (síntoma: el sidebar cambia y la tabla no; parecía fuga cross-org y era estado cliente rancio). El listener de `BroadcastChannel('facturaia.active-org')` recarga igual en las otras pestañas. Ver [[router-refresh-no-remonta-los-client-components-datos-de-la-org-anterior]]
