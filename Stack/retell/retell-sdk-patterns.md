...
- **No existe endpoint de UPDATE de un caso** (medido 18-sep: `PATCH /update-test-case-definition/<id>` da 404 en todas las variantes — id en la ruta, id en el cuerpo, `/v2/`, singular y plural). Editar un criterio = `POST /create-test-case-definition` (201) + `DELETE /delete-test-case-definition/<id>` (204, probado primero contra uno recién creado tuyo). **El id cambia**, así que cualquier script o runbook que fije ids hay que tocarlo en la misma pasada.
- Los criterios del caso viven en `metrics`; el estado de cada corrida es `pass` / `fail` / **`error`** — `error` es «la simulación no llegó a correr», no un fallo del agente: no lo cuentes como rojo.
- **Leer un flujo: el enrutado NO está solo en `edges`.** Un nodo `extract_dynamic_variables` sale por `else_edge`, y hay además `skip_response_edge` y `global_node_setting`. Buscando solo `edges`, un nodo de función parece huérfano y se diagnostica un bug estructural que no existe (18-sep, a punto de publicarlo). Vuelca las **claves** del nodo antes de concluir nada sobre alcanzabilidad.
