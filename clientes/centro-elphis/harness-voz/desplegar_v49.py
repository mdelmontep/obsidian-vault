"""Despliega el candidato v49 del agente de voz. NO se ejecuta solo: hay que pasarle --publicar
y luego --pin. Cada paso verifica contra el servidor y aborta si produccion se movio.

Paso 1 (sin flags): crea el borrador desde base 46, empuja el flow y pasa el gate.
...
if "--pin" in sys.argv:
    nv=int(open(f"{S}/.v49_version").read().strip())
    ...
    print(f"ROLLBACK: el mismo UPDATE con 46 en vez de {nv}.")
    sys.exit(0)
...
assert ddi==46, f"el DDI no esta en 46 sino en {ddi}: parar y mirar"
g=json.load(open(CAND,encoding="utf-8"))
v=api("POST",f"/create-agent-version/{AG}",{"base_version":46})
nv=v["response_engine"]["version"]
print(f"borrador creado: v{nv} (NO publicado), base=v46")
...
assert ddi_version()==46, "el DDI se movio durante el proceso"
json.dump(srv,open(f"{S}/flow_v{nv}_srv.json","w",encoding="utf-8"),ensure_ascii=False)
open(f"{S}/.v49_version","w").write(str(nv))
print(f"verificado contra el servidor. produccion intacta (publicada v{desp}, DDI v46).")
