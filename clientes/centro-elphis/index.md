...
- ✅ **Voz v49 en producción** (13-sep, 14:05). Publicada 49 · pin 49 · DDI 49, verificadas contra la API. = v48 + horario: la v48 decía *"te lo confirma recepción"* y dos turnos después *"de 9 a 21 horas"* (llamada de Manu `call_f27129552274466c52afa564ebd`). Solo `global_prompt`: frase literal en horas de reloj, instrucción pegada al dato y prohibido solo decir si está abierto AHORA. `patch_r49_horario.py` · `desplegar_r49_horario.py` · caso nuevo HO `test_case_06dbe35ccc81` · suite `batch-r49.json`: **39/39 vs 33/39** de la v48 en 3 rondas alternas, HO 3/3 vs 0/3. Rollback: `ROLLBACK.sh 48`. → [[prohibir-el-dato-fijo-para-evitar-una-deduccion-enfrenta-la-regla-con-la-base]]
  - ⚠️ **Cada despliegue correcto dispara el rojo de T1** («el pin del teléfono de Laura había cambiado… se hizo al revés»): no distingue subir la fila de mover Retell a mano. Tell: `created_at` del pin <5 min antes del aviso y `observado` = pin anterior.
  - 🔜 **Tuyo**: borrar el deal de prueba `32122661` (reabierto por esa llamada) y devolver el nombre «Agentesia prueba e2e» a la ficha `161749243`, más la clave `deal-open-161749243`; decirle a Alba que el WhatsApp + email «nuevo lead Manuel Agentecia» de las 13:45 era una prueba. **Una llamada de prueba con nombre y motivo registra y avisa aunque no acepte cita.**
  - 🔜 Falta tráfico real con la v49 (horario y el caso de crisis).
- ✅ **Voz v48** (13-sep, mañana; sustituida por la v49). Publicada 48 · pin Postgres 48 · DDI `inbound_agents[{agent_version: 48}]`, las tres coherentes y verificadas contra la API. Gate contra la versión **publicada**: 39 inviolables, 0 fallos.
...
- ✅ **`gate.py` vuelve a medir** (9-sep). Llevaba desde el 7-sep dando **los mismos 33 fallos contra v37, v44 y v45** porque su base era `flow-P6.json`, un diseño que nunca se publicó. Ahora son dos categorías: `check()` mide contra `snapshots/flow-PROD-v45.json` y bloquea (30), `deuda()` lista lo de F4 y no bloquea (24). **Verificado que discrimina**, que es la prueba que faltaba: v45 → 0 fallos / 24 deudas · v31 y v29 → 2 fallos · P6 → 14 fallos / 0 deudas. Dato que sale de ahí: **desplegar F4 tal cual rompe 14 inviolables vigentes**. El consentimiento en `prompt` figura como deuda pero es deliberado desde el 27-ago (como `static_text` recitaba sus directrices en voz alta). → [[un-gate-anclado-a-un-diseno-no-desplegado-falla-identico-contra-todo-y-deja-de-medir]]
...
- ✅ **El `+34687448210` es de Alba** (confirmado por Manu el 9-sep, cierra la duda abierta desde el 10-ago). Es el destino de `ELPHIS_NOTIF_INGRESO` y `ELPHIS_NOTIF_RECEPCION`; el email va al buzón genérico `info@centroelphis.com`, que es el que ve Olga. `ELPHIS_NOTIF_PRINCIPAL` está vacío.
...
- ✅ **Los duplicados, cerrados por los dos lados.** Bot contra sí mismo: paso 2.7 (12:26) + fix de idempotencia (13:13) — `NULLIF` contra `{"deal_id":null}` porque el `COALESCE` no filtra un jsonb, y ventana fija por episodio. Verificado con tráfico real: 3 pacientes, 7 ejecuciones, 3 deals. Bot contra recepción (22:45): `Check deal abierto` ya consulta Clientify y **adopta** el deal que hizo Olga en vez de crear otro; 50 puertas, 24 mutantes. Si Clientify falla, *fail closed*: no crea deal pero **sí avisa** a recepción. → [[on-conflict-coalesce-no-filtra-un-jsonb-null-y-el-where-veta-el-update]] · [[un-mutante-sobrevive-cuando-los-datos-reales-tapan-un-brazo-del-or]]
- ✅ **Guarda del deal cerrado (21:37)**: `Resolver acción` ya no resucita un deal que recepción cerró; 48 puertas, la J07 ejecutada contra los 1.434 deals reales.
- ✅ **Embudo 56886 reordenado**: 0..6 alineado con el RANK del código; censo de 1.570 deals idéntico. → [[el-orden-visual-del-embudo-y-el-rango-del-codigo-son-dos-verdades]]
- ✅ **Retell no puede pedir el trabajo dos veces** (22:45): candado `postcall-<call_id>`, 7 días, y 200 al duplicado (un error haría reintentar más). Bug demostrado, no supuesto: execs 10917/10923 del 28-ago con el mismo `call_id` porque la primera tardó 9,1 s. Aquí el `WHERE expires_at < NOW()` **sí es correcto** — es un candado puro que no guarda respuesta, al revés que en `registrar-lead`.
- ✅ **`error-handler-global`, tres defectos**: código duplicado en el aviso (regresión del 7-sep entre 18:44 y 18:59 UTC — *no* la fecha del `updatedAt`), clave de dedup saturada a 40 chars → firma del detalle entero, y el emisor real recuperado del inventario de los 6 nodos que invocan `avisar-fallo`.
- ✅ **`crisis-reconciler` blindado y SIGUE PARADO** a propósito: 0 llamadas perdidas en 30 días, así que no hay urgencia. Su filtro por `agent_id` ya estaba y vale oro: la cuenta de Retell tiene **11 agentes de 8 clientes**, y sin filtro entrarían 179 llamadas de pacientes de otras clínicas al CRM de Elphis (art. 9 RGPD). Su agujero real era la idempotencia: un INSERT fallido lo hacía reenviar cada 10 min → 144 emails de crisis/día.
- ✅ **Notas de la cita (F3.9)**: anclado de la línea `Cita Doctoralia` — el recorte viejo la perdía tras 15 mensajes de seguimiento.
- ✅ **Rastro limpio**: `zz-p4-runner` (webhook público de pruebas capaz de disparar Slack) apagado, y el deal de prueba `32288770` cerrado. Verificado que **ningún workflow de producción llamaba a un mock**.
