...
- **Los avisos internos del bot llevaban meses sin llegar al equipo, por dos causas independientes. Ambas cerradas y verificadas E2E hoy.**
  - **Iban al móvil del dev, no a recepción**: `ELPHIS_NOTIF_INGRESO`/`_RECEPCION` seguían con el `+34617314938` de staging porque el «número directo de la recepcionista» figuraba como pendiente en [[bloqueantes-elphis]]… cuando **el número de Alba ya estaba en este hub desde mayo** (§Datos clave). Ahora los dos en **`+34687448210`** (Alba) y `ELPHIS_NOTIF_EMAIL` en `info@centroelphis.com`, verificado con `printenv` dentro del container.
  - **El 89% de los leads no avisaba a nadie**: el `else` de `Decidir etapa` ponía `destino='none'`, y ahí caían el default `'info'` y cualquier `tipo_consulta` que inventara el LLM → 65 de 73 leads en 14 días, sin un solo error y con todo en `success`. Cambiado ese `else` a `destino='recepcion'` manteniendo `stage_id=243848` (el CRM no se toca). Ver [[el-else-de-un-clasificador-que-rellena-un-llm-debe-avisar-no-callar]] — es la **secuela directa del fix del 22-jul**, que ató la notificación justo a `destino !== 'none'`.
  - **Verificado E2E** por el webhook `retell/crear-lead` con `tipo_consulta` inventado y teléfono de whitelist, apuntando el env temporalmente al móvil/correo de Manu y revirtiéndolo después: `wamid` de Meta + `250 2.0.0` del SMTP, y `Enviar notif`/`Enviar email notif` en `t|t` (la ejecución anterior, `f|f`).
  - Volumen real, contando **teléfonos distintos y no ejecuciones** (`registrar-lead` se llama ~10 veces por conversación, y el dedup agrupa por teléfono+hora): 13 personas en 14 días, máximo **4 en un día**. Recepción pasa de 3 avisos en 14 días a ~1/día con picos de 4 — nada alarmante. Ojo al medir esto: contar ejecuciones da 4-5/día y un pico de 32, que es falso.
  - `notify-cita-confirmada-email` (`3WeyF5AdR4vWJbqO`) **desactivado**: huérfano desde el rediseño de junio (el paciente elige hueco en Doctoralia), 0 workflows lo referencian. Ver [[workflow-activo-no-significa-llamado-grep-su-id-antes-de-editarlo]].
  - **Limpiar en Clientify**: lead de prueba de hoy, contacto `170386983` / deal `31693026` (`+34600000090`).
...
- 🔴 **`chatwoot-event` falla 1 de cada 5 ejecuciones (detectado 3-ago, SIN diagnosticar — **sigue igual el 10-ago**: 73 errores de 379 ejecuciones, 19%)** — 65 ejecuciones en ERROR en 7 días (de ~300), la última el 3-ago 18:29. Nodo `Persist ids conv_state`, `invalid input syntax for type bigint: "null"` — llega un `"null"` como STRING donde se espera un bigint. Los nodos ejecutados llegan hasta `Lock eval`, así que revienta DESPUÉS de coger el lock de conversación. Es el bot vivo del 659 con pacientes reales. Salió del check de efecto ([[agentes-cliente-tres-capas]]), no del error handler: `error-handler-global` existe y nadie mira lo que reporta. Siguiente paso: qué id llega como `"null"` y de dónde sale.
