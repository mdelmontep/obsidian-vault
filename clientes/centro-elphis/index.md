...
- **Bot de chat respondía a preguntas de composición/efectos de sustancias (reportado por Alba: contestó que el tussi lleva heroína) + no avisaba al equipo cuando alguien decía "ya tuve visita/ya hablé con vosotros" — ambos corregidos y desplegados en `router-ia` (`7huQC9GWl12SpYaE`).**
  ...
  - **Voz también corregida** (agente `Laura — Centro Elphis (voz) - Flow v4`, `agent_e21120298343bc2ef8b4a535c9`, conversation flow `conversation_flow_a42bf76dcfa0`, número real **`+34910054950`** — corrige el supuesto anterior de que era el número de pruebas Agentesia). Mismo bloqueo de composición de sustancias en el `global_prompt` (publicado, versión agente v2). "Contacto previo" ampliado en los edges `welcome`/`intake` → `datos_mensaje` (que ya recoge mensaje + promete contacto + llama `crear_lead`) + 2 ejemplos few-shot — publicado igual. Retell Conversation Flow no tiene capa de código determinista (las transiciones son siempre `type: prompt`, evaluadas por el modelo), así que aquí no hay garantía dura como en `router-ia`; es la mejor aproximación disponible en esa arquitectura.
  - ⚠️ **La API key de Retell (compartida por TODOS los clientes de Agentesia, no solo Elphis) se pegó en texto plano en este chat de Claude Code (04-ago)** porque no hay ninguna credencial de Retell en el vault `Elphis` de 1Password — solo el login del dashboard (`Retell AI`, vault `Agentesia`, usuario/contraseña) en el vault `Agentesia` es lo que existía. Otros clientes sí tienen su propio item `Retell API` (Clínica Zen, EcoBox, Proyecto Tecnocloud) pero Elphis no. Pendiente: (1) decidir si rotar esa key (afecta a TODOS los agentes Retell de la agencia, no solo Elphis — evaluar impacto antes); (2) en cualquier caso, guardar una copia limpia en 1Password (vault `Agentesia` o `Compartida Agentesia`, item `Retell API`) en vez de dejarla solo en el historial del chat.
